...
if (!ALF_Killed && {!isNil "tmp_pending_respawn"} && {tmp_pending_respawn isEqualType true} && {tmp_pending_respawn}) then {
	tmp_pending_respawn = nil;
	[] spawn {
		sleep 0.1;
		[] call ALF_fnc_spawnMenu;
	};
};
...
if (ALF_Killed) then {
	_this spawn {
		...
		_unit setVariable ["phoneNumber",ALF_Phone_Number,2];
		...
		if (isPlayer life_killer && {_unit != life_killer}) then {
			[name _unit,getPlayerUID _unit,"MORT",format["a été tué par %1 [%2m avec %3]",name life_killer,_unit distance life_killer,currentWeapon life_killer]] remoteExec ["ALF_Server_fnc_logIt",2];
		...
		_unit spawn {
			...
			private "_maxTime";
			if (alf_adminlevel < 3) then 
			{
				if (alf_ms_isdead) then 
				{
					_maxTime = time + 120;//120
				} else {
					if(ALF_Count_Meds isEqualTo 0) then 
					{
						_maxTime = time + 300;//5 minutes
					} else 
					{
						_maxTime = time + 900;//15 minutes
					};
				};
			} else
			{
		...
		[] remoteExecCall ["ALF_Server_fnc_deathScreen",2];
