...
// Integration appuie touche Tel MRP
if (_shift AND {!_ctrlKey AND { _code isEqualTo 21}}) then {[] call MRPV2_fnc_MRP_Tel_Init_Keydown;};
...
// Poker géré via addAction (GetInMan EH dans initPlayer) - plus de keybind ici
...
if (ALF_PoserPerceuseActive) exitWith {
	switch (_code) do {
		...
		case 1: {
			_handled = true;
			if !(player canAdd "ALF_Perceuse_GDM_M") exitWith {["Perceuse", "Vous ne pouvez pas remettre la perceuse dans votre inventaire", "danger"] spawn ALF_fnc_doMsg;};
			...
			["ALF_Perceuse_GDM_M",true] spawn ALF_fnc_handleItem;
		...
		case 57: {
			_handled = true;
			if (isSimpleObject ALF_PoserChargeHouse && {!(((getModelInfo _curObject) select 1) isEqualTo "alf_mobiliers\alf_safe_1\alf_safe_1.p3d")}) exitWith {
				["Perceuse", "Vous devez placer une perceuse sur un coffre.", "danger"] spawn ALF_fnc_doMsg;
			};
			...
			[] spawn {
				...
				// ----- DÉTERMINER SI C'EST UN COFFRE FORT OU UNE PORTE -----
				private _target = ALF_PoserChargeHouse;
				private _isSafe = false;
				
				if (!isNull _target) then {
					private _modelPath = (getModelInfo _target) select 1;
					if (!isNil "_modelPath" && {_modelPath isEqualTo "alf_mobiliers\alf_safe_1\alf_safe_1.p3d"}) then {
						_isSafe = true;
					};
				};

				if (_isSafe) then {
					// ----- LOGIQUE COFFRE FORT : percer le coffre -----
					private _alreadyPerced = _target getVariable ["perced", false];
					
					if (_alreadyPerced) then {
						["Perceuse", "Ce coffre a déjà été percé.", "warning"] spawn ALF_fnc_doMsg;
					} else {
						// Marquer le coffre comme percé localement (variable synchronisée)
						_target setVariable ["perced", true, true];
						
						// Mettre à jour mobilier_data si elle existe (pour cohérence)
						private _mobilierData = _target getVariable ["mobilier_data", []];
						if (count _mobilierData >= 6) then {
							_mobilierData set [5, true];
							_target setVariable ["mobilier_data", _mobilierData, true];
						};
						
						["Perceuse", "Le coffre fort a été percé avec succès.", "success"] spawn ALF_fnc_doMsg;
					};
				} else {
					// ----- LOGIQUE PORTE : déverrouiller + ouvrir -----
					private _house = _target;
					private _locked = _house getVariable ["ALF_PorteFermer", false];

					if !(_locked) then {
						["Perceuse", "La porte est déjà déverrouillée.", "danger"] spawn ALF_fnc_doMsg;
					} else {
						[_house] spawn {
							params ["_house"];

							// Index de la porte — fallback propre si non défini
							private _door = missionNamespace getVariable ["ALF_PoserDoorIndex", 1];
							if (!(_door isEqualType 0)) then { _door = 1; };
							_house setVariable ["ALF_PorteFermer", false, true];
							_house setVariable [format["bis_disabled_Door_%1", _door], 0, true];
							private _srcName = format["Door_%1_source", _door];
							private _rotName = format["Door_%1_rot", _door];
							_house animateSource [_srcName, 1];
							uiSleep 0.05;
							_house animate [_rotName, 1];

							["Perceuse", "La porte a été ouverte.", "success"] spawn ALF_fnc_doMsg;

							// ---- Déclenchement breakdoor (alarme + marquage + visages) ----
							_house setVariable ["ALF_BreakDoor", true, true];
					
							private _pos = getPosATL player;
							[_pos] spawn ALF_fnc_robberyAlert;
						};
					};
				};
...
if (ALF_PoserChargeActive) exitWith {
	switch (_code) do {
		...
		case 1: {
			_handled = true;
			if !(player canAdd "ALF_Charge_C2_M") exitWith {["Poser Charger", "Vous ne pouvez pas remettre votre charge dans votre inventaire", "danger", false] spawn ALF_fnc_doMsg;};
			deleteVehicle ALF_PoserChargeObject;
			ALF_PoserChargeObject = objNull;
			removeMissionEventHandler ["EachFrame",ALF_EventHandlerPoserCharge];
			ALF_EventHandlerPoserCharge = -1;
			["ALF_Charge_C2_M",true] spawn ALF_fnc_handleItem;
		...
		case 57: {
			...
			private _door = [ALF_PoserChargeHouse,(ASLToAGL (getPosASL ALF_PoserChargeObject))] call ALF_fnc_nearestDoor;
...
if (ALF_PoserViandeActive) exitWith {
	switch (_code) do {
		...
		case 1: {
			...
			if !(player canAdd _item) exitWith {["Poser Viande", "Vous ne pouvez pas remettre votre viande dans votre inventaire", "danger", false] spawn ALF_fnc_doMsg;};
		...
		case 57: {
			...
			ALF_EventHandlerPoserViande = -1;
			ALF_PoserViandeActive = false;
			[ALF_PoserViandeObject,ALF_PoserViandeBBQ] remoteExec ["ALF_Server_fnc_cuireViande",2];
			ALF_PoserViandeObject = objNull;
...
if (life_mobilier_active) exitWith {
	switch (_code) do {
		//space key
		case 57: {
			[] spawn ALF_fnc_placeMobilierAction;
			_handled = true;
		};
		//FLECHE UP
		case 200: {
...
switch (_code) do {
	...
	case 35: 
	{
		...
		if(!_shift && {!_ctrlKey} && {"O2_Item_Stop_Stick_x2" in (magazines player)} && {(vehicle player) isEqualTo player}) then {
			["O2_Item_Stop_Stick_x2"] spawn ALF_fnc_spikeStrip;
		};
	};
	...
	{		
        ...
        if (_shift) then 
		{
            [_veh, "toggleReg"] call MRPV2_fnc_Vehicule_Regulateur_Limiteur;
        } else 
		{
            [_veh,"toggleLimiter" ] call MRPV2_fnc_Vehicule_Regulateur_Limiteur;
        };
    };
};
