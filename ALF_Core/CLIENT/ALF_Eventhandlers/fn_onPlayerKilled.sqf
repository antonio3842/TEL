...
if (isPlayer life_killer && !(_unit != life_killer)) then {
	if((vehicle _unit) != _unit) then {
		[name _unit,getPlayerUID _unit,"MORT",format["Il s est suicidé a bord de %1.",typeOf(vehicle _unit)]] remoteExec ["ALF_Server_fnc_logIt",2];
	} else {
		[name _unit,getPlayerUID _unit,"MORT","Il s est suicidé."] remoteExec ["ALF_Server_fnc_logIt",2];
	};
};
...
if ((((getAllHitPointsDamage _unit) select 2) select 2 > 0.8)) then {};
...
_unit setVariable ["bouteilleo2",alf_ms_bouteille,2];
//};
