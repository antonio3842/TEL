...
switch (_mode) do {
	...
	case 0: {
		...
		[_chaise] remoteExec ["ALF_Server_fnc_donDuSang",2];
	...
	case 1: {
		...
		private _action = ["Voulez-vous retirer la perfusion?","MEDICAL","OUI","NON"] call BIS_fnc_guiMessage;
		if (!_action) exitWith {};
		...
		[_classNamePoche,true] spawn ALF_fnc_handleItem;
		_chaise setObjectTextureGlobal [0, "ALF_CentrePompier\ALF_Don_Sang\Sang\Perfusion_neuve.paa"];
