...
switch (_item) do {
	case 0 : {
		...
		["INFO", "Vous avez bien appliqué le bandage.", "success"] spawn ALF_fnc_doMsg;
	};
	case 1 : {
		["ALF_Saline",false] spawn ALF_fnc_handleItem;
		["ALF_Saline",0] remoteExec ["ALF_fnc_medicalItems",_unit];
		["INFO", "Vous avez bien appliqué la saline.", "success"] spawn ALF_fnc_doMsg;
	};
	case 2 : {
		["ALF_M_Oxy9L",false] spawn ALF_fnc_handleItem;
	...
	case 3 : {
		...
		["ALF_Adrenaline",false] spawn ALF_fnc_handleItem;
		["ALF_Adrenaline",(parseNumber _dose)] remoteExec ["ALF_fnc_medicalItems",_unit];
		["INFO", "Vous avez bien injecté l'adrénaline.", "success"] spawn ALF_fnc_doMsg;
	};
	case 4 : {
		private _dose = ctrlText 18013;
		if(!([_dose] call TON_fnc_isnumber)) exitWith {hint "La dose n'est pas correcte.";};
		if((parseNumber _dose) > 10 OR {(parseNumber _dose) <= 0}) exitWith {hint "La dose n'est pas correcte.";};
		["ALF_Atropine",false] spawn ALF_fnc_handleItem;
		["ALF_Atropine",(parseNumber _dose)] remoteExec ["ALF_fnc_medicalItems",_unit];
		["INFO", "Vous avez bien injecté l'atropine.", "success"] spawn ALF_fnc_doMsg;
	};
	case 5 : {
		private _dose = ctrlText 18013;
		if(!([_dose] call TON_fnc_isnumber)) exitWith {hint "La dose n'est pas correcte.";};
		if((parseNumber _dose) > 10 OR {(parseNumber _dose) <= 0}) exitWith {hint "La dose n'est pas correcte.";};
		["ALF_Morphine",false] spawn ALF_fnc_handleItem;
		["ALF_Morphine",(parseNumber _dose)] remoteExec ["ALF_fnc_medicalItems",_unit];
		["INFO", "Vous avez bien injecté la morphine.", "success"] spawn ALF_fnc_doMsg;
	};
	case 6 : {
		...
		if (_classnamepoche isEqualTo "") exitwith {["INFO", "Erreur : vous n'avez plus de poche de sang sur vous.", "danger"] spawn ALF_fnc_doMsg;};


		[_classnamepoche,false] spawn ALF_fnc_handleItem;
		[_classnamepoche,0] remoteExec ["ALF_fnc_medicalItems",_unit];
		["INFO", "Vous avez bien appliqué la poche de sang.", "success"] spawn ALF_fnc_doMsg;
	};
};
