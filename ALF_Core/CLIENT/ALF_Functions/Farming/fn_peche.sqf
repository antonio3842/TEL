...
// ["ALF_appat",true] spawn ALF_fnc_handleItem;
...
// [_p,true] spawn ALF_fnc_handleItem;
// ALF_PecheEnCours = false;
...
/* === LOOT RNG SANS NIVEAUX / SANS XP — TABLE FIXE === */
private _lootTable = [
    ["ALF_Maquereau_Crue_M", 0.30],
    ["ALF_Sardine",          0.25],
    ["ALF_Thon",             0.15],
    ["ALF_Barbeau",          0.10],
    ["ALF_Brochet",          0.08],
...
/* Sélection pondérée */
private _totalW = 0;
{ _totalW = _totalW + (_x select 1); } forEach _lootTable;

private _r = random _totalW;
private _acc = 0;
private _p = (_lootTable select 0) select 0; // fallback
{
    _acc = _acc + (_x select 1);
    if (_r <= _acc) exitWith { _p = _x select 0; };
} forEach _lootTable;
