...
*/
if (life_permis_action) exitWith {["Moniteur Auto-Ecole", "Tu passes dejà votre permis.", "danger", false] spawn ALF_fnc_doMsg;};
if (life_permis_time) exitWith {["Moniteur Auto-Ecole", "Vous ne pouvez pas passer votre stage de récupération tout de suite. Vous ne pouvez passer de stage que toute les 24H.", "danger", false] spawn ALF_fnc_doMsg;};
private _desactiver = false;
{if (_x getVariable["ALF_Moniteur", false]) exitWith {_desactiver = true}} forEach playableUnits;

if (_desactiver) exitWith {["Auto-Ecole", "Il y a l'entreprise auto-école d'ouverte rend vous labas !!", "danger", false] spawn ALF_fnc_doMsg;};

_price = 925;

_action = [format["Voulez-vous passer votre stage de récupération de points pour %1€. Attention, epreuve sous 5 erreurs eliminatoires.",_price],"Inscription au stage","OUI","NON"] call BIS_fnc_guiMessage;
...
if(_action) then {
    ...
    if (_exit) exitWith {hint "Vous avez râte votre permis, le moniteur est parti."};
