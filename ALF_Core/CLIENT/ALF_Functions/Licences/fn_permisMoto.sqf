...
*/
/*
if !(license_driver) exitWith {["Moniteur Auto-Ecole", "Vous devez déjà avoir le permis de conduire.", "danger", false] spawn ALF_fnc_doMsg;};
if (license_moto) exitWith {["Moniteur Auto-Ecole", "Vous avez déjà le permis Moto.", "danger", false] spawn ALF_fnc_doMsg;};
if (life_permis_action) exitWith {["Moniteur Auto-Ecole", "Vous passez déjà votre permis.", "danger", false] spawn ALF_fnc_doMsg;};
if (life_permis_time) exitWith {["Moniteur Auto-Ecole", "Vous ne pouvez passer un stage que toute les 72H.", "danger", false] spawn ALF_fnc_doMsg;};
if !((headgear player) in ["ALF_Casque_PMO","casque_modulable_simple","ALF_Bike","ALF_H_Moto_2_Violet","ALF_H_Moto_2_Vert","ALF_H_Moto_2_RougeFonce","ALF_H_Moto_2_Orange","ALF_H_Moto_2_Jaune","ALF_H_Moto_2_Bleu","ALF_H_Moto_2_Blanc","ALF_H_Moto_2_Noir","ALF_H_Moto_Violet","ALF_H_Moto_Vert","ALF_H_Moto_RougeFonce","ALF_H_Moto_Orange","ALF_H_Moto_Jaune","ALF_H_Moto_Bleu","ALF_H_Moto_Blanc","ALF_H_Moto_Noir"]) exitWith {["Moniteur Auto-Ecole", "Il vous faut un casque.", "warning", false] spawn ALF_fnc_doMsg;};

_price = 1200;

_action = [format["Voulez-vous passer votre permis moto pour %1€. Attention, l'épreuve est éliminatoire à 5 erreurs.",_price],"Inscription au permis moto","OUI","NON"] call BIS_fnc_guiMessage;
...
if(_action) then {
    ...
    if(life_cash < _price) exitWith {["Moniteur Auto-Ecole", "Vous n'avez pas assez d'argent sur vous.", "danger", false] spawn ALF_fnc_doMsg;};
    ...
    ["Moniteur Auto-Ecole", "Le moniteur est prêt à vous guider a distance, vous pouvez monter sur la moto.", "warning", false] spawn ALF_fnc_doMsg;
    ...
    if (_exit) exitWith {hint "Vous avez raté votre permis, le moniteur c'est déconnecté."};
    ...
    if (_exit) exitWith {life_permis_action = false; player allowDamage false; [] spawn {sleep 5; player allowDamage true;}; deleteVehicle _vehicle; player setPosATL _startPos; ["Moniteur Auto-Ecole", "Vous avez râte votre permis, vous êtes partie trop loin.", "warning", false] spawn ALF_fnc_doMsg;};
    ...
    for "_i" from 0 to 1 step 0 do {
        _speed = round speed (vehicle player);
        if (round _speed > 100 && _erreur isEqualTo _myerreur) then {_erreur = _erreur + 1; hint "Attention, vous allez trop vite, vous venez de refaire une erreur"};
    ...
    for "_i" from 0 to 1 step 0 do {
        _speed = round speed (vehicle player);
        if (round _speed > 70 && _erreur isEqualTo _myerreur) then {_erreur = _erreur + 1; hint "Attention, vous allez trop vite, vous venez de refaire une erreur"};
    ...
    for "_i" from 0 to 1 step 0 do {
        _speed = round speed (vehicle player);
        if (round _speed > 120 && _erreur isEqualTo _myerreur) then {_erreur = _erreur + 1; hint "Attention, vous allez trop vite, vous venez de refaire une erreur"};
    ...
    for "_i" from 0 to 1 step 0 do {
        _speed = round speed (vehicle player);
        if (round _speed > 75 && _erreur isEqualTo _myerreur) then {_erreur = _erreur + 1; hint "Attention, vous allez trop vite, vous venez de refaire une erreur"};
...
if(license_moto) exitWith {["INFO","Vous avez deja le Permis moto.","warning"] spawn ALF_fnc_doMsg;};

private _action = ["Voulez-vous acheter un Permis moto pour 1200€.","Le Permis Moto","OUI","NON"] call BIS_fnc_guiMessage;
if(_action) then {

    if(life_cash < 1200) exitWith {["INFO","Vous n'avez pas 1200€ sur vous.","warning"] spawn ALF_fnc_doMsg;};

    life_cash = life_cash - 1200;
    [0] call SOCK_fnc_updatePartial;

    license_moto = true;
    [1] call SOCK_fnc_updatePartial;

    ["INFO","Vous avez acheté le Permis moto d'une valeur de 1200€.","success"] spawn ALF_fnc_doMsg;
};
