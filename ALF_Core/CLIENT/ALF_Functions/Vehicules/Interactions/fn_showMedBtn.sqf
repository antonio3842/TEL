...
_Btn2 buttonSetAction "[life_vInact_curTarget] spawn ALF_fnc_pulloutActionsp; closeDialog 0;";

private _locked = locked _veh;

if(b_Depanneur && !life_pompier_service) then {
	_Btn3 ctrlShow true;
	if(_locked isEqualTo 2) then {
		_Btn3 ctrlSetText "[DEP] Ouvrir véhicule";
		_Btn3 buttonSetAction "[life_vInact_curTarget,0] remoteExecCall [""ALF_fnc_lockVehicle"",life_vInact_curTarget]; closeDialog 0;";
	} else {
		_Btn3 ctrlSetText "[DEP] Fermer véhicule";
		_Btn3 buttonSetAction "[life_vInact_curTarget,2] remoteExecCall [""ALF_fnc_lockVehicle"",life_vInact_curTarget]; closeDialog 0;";
	};
} else {
