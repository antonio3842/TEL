...
_R3 buttonSetAction "[life_vInact_curTarget] spawn ALF_fnc_pushObject; closeDialog 0;";
if (alive _veh && {local _veh} && {crew _veh isEqualTo []}) then {_R3 ctrlShow true;} else {_R3 ctrlShow false};

if (life_gendarme_service || {b_Depanneur}) then {
	_R4 ctrlShow true;
	...
	if ((speed life_vInact_curTarget) isEqualTo 0 && {!(life_vInact_curTarget in (missionNamespace getVariable ["VehiculeImmobiliser",[]]))} && {(count (crew life_vInact_curTarget)) isEqualTo 0} && {life_gendarme_service}) then {
		_R4 buttonSetAction "[life_vInact_curTarget] call ALF_fnc_immobiliserVehicle; closeDialog 0;";
	} else {
