...
if(life_interrupted) exitWith {life_interrupted = false; [_item,true] spawn ALF_fnc_handleItem; ["Siphonnage", "Action annulée.", "danger"] spawn ALF_fnc_doMsg;};
...
switch (true) do
{
	...
	{
		["ALF_Barile_10",true] spawn ALF_fnc_handleItem;
	};
	
	case (fuel _vehicle <= 0.3 && fuel _vehicle > 0.2):
	{
		["ALF_Barile_20",true] spawn ALF_fnc_handleItem;
	};
	
	case (fuel _vehicle <= 0.4 && fuel _vehicle > 0.3):
	{
		["ALF_Barile_30",true] spawn ALF_fnc_handleItem;
	};
	
	case (fuel _vehicle <= 0.5 && fuel _vehicle > 0.4):
	{
		["ALF_Barile_40",true] spawn ALF_fnc_handleItem;
	};
	
	case (fuel _vehicle <= 0.6 && fuel _vehicle > 0.5):
	{
		["ALF_Barile_50",true] spawn ALF_fnc_handleItem;
	};
	
	case (fuel _vehicle <= 0.7 && fuel _vehicle > 0.6):
	{
		["ALF_Barile_60",true] spawn ALF_fnc_handleItem;
	};
	
	case (fuel _vehicle <= 0.8 && fuel _vehicle > 0.7):
	{
		["ALF_Barile_70",true] spawn ALF_fnc_handleItem;
	};
	
	case (fuel _vehicle <= 0.9 && fuel _vehicle > 0.8):
	{
		["ALF_Barile_80",true] spawn ALF_fnc_handleItem;
	};
	
	case (fuel _vehicle <= 1 && fuel _vehicle > 0.9):
	{
		["ALF_Barile_90",true] spawn ALF_fnc_handleItem;
	};
	
	case (fuel _vehicle == 1):
	{
		["ALF_Barile_100",true] spawn ALF_fnc_handleItem;
	};
	
};

["Bidon", "Vous avez rempli votre bidon.", "success"] spawn ALF_fnc_doMsg;
[name player,getPlayerUID player,"VEHICULE",format["A siphonné %1 | Pos : %2 | MapPos : %3 | Fuel : %4", getText(configFile >> "CfgVehicles" >> (typeOf _vehicle) >> "displayName"), getPos _vehicle, mapGridPosition _vehicle, fuel _vehicle]] remoteExec ["ALF_Server_fnc_logIt",2];
...
{
	[_vehicle,0] remoteExec ["ALF_fnc_setFuel",_vehicle];
}
else
{
	_vehicle setFuel 0;
...
if(_dice > 200) then {
	["INFO", "Vous avez déclenché l'alarme du véhicule !", "warning"] spawn ALF_fnc_doMsg;
	[_vehicle,"ALF_CarAlarm1"] remoteExec ["ALF_fnc_say3D",RANY];
};
