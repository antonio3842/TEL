...
switch (_mode) do {
	case 0 : {
		...
		_pgText ctrlSetText format["Remplacement d'une roue (1%1)...","%"];
		...
		for "_i" from 0 to 1 step 0 do {
			...
			_pgText ctrlSetText format["Remplacement d'une roue (%1%2)...",round(_cP * 100),"%"];
		...
		player switchMove "";
		["Réparation", "La roue est remplacée.", "success", false] spawn ALF_fnc_doMsg;
		["ALF_Roue",false] spawn ALF_fnc_handleItem;
		detach _3dobj;	
		deletevehicle _3dobj;		

		private _lvl = 0.5;
		private _wheels = ["HitLFWheel","HitLF2Wheel","HitRFWheel","HitRF2Wheel"];
		...
		if !(local _veh) then {
			{
				if (!_repaired) then {
					private _currentDamage = _veh getHitPointDamage _x;
	...
	case 1 : {
		...
		["ALF_Boiteaoutils",false] spawn ALF_fnc_handleItem;
		detach _3dobj;	
		deletevehicle _3dobj;
	...
	case 2 : {
		...
		_progress = _ui displayCtrl 38201;
		_pgText = _ui displayCtrl 38202;
		_pgText ctrlSetText format["Réparation méchanique (1%1)...","%"];
		_progress progressSetPosition 0.01;
		_cP = 0.01;
		_speed = 0.01;
		...
		for "_i" from 0 to 1 step 0 do {
			...
			_cP = _cP + 0.01;
			_progress progressSetPosition _cP;
			_pgText ctrlSetText format["Réparation méchanique (%1%2)...",round(_cP * 100),"%"];
			if(_cP >= 1) exitWith {detach _3dobj;	deletevehicle _3dobj;};
			if(!alive player) exitWith {detach _3dobj;	deletevehicle _3dobj;};
			if(player != vehicle player) exitWith {detach _3dobj;	deletevehicle _3dobj;};
			if(life_interrupted) exitWith {detach _3dobj;	deletevehicle _3dobj;};
		...
		["ALF_KitMecatechnic",false] spawn ALF_fnc_handleItem;
		detach _3dobj;	
		deletevehicle _3dobj;
