...
if(b_1 || b_33) then {_price = 1000;};

private _action = [format["Voulez-vous transferer ce vehicule dans l'entreprise ? Le montant du transfert est de %1 €.", _price],"Garage","OUI","NON"] call BIS_fnc_guiMessage;
...
if (_action) then {
	...
	[_vid,(getPlayerUID player),_owner,_siretstr] remoteExecCall ["ALF_Server_fnc_vehicleTransfert",2];
};