...
if(_c > 0) then {
	...
	private _a = [format["Le véhicule est rempli de %1 sachet(s) de cocaine d'une valeur de %2€. Voulez-vous les saisir ? Cela enlevera tout ce qu'il y a dans le coffre.",_cf,[_pf] call ALF_fnc_numberText],"VEHICULE","OUI","NON"] call BIS_fnc_guiMessage;
	if !(_a) exitWith {};
	...
	[name player,getPlayerUID player,"VEHICULE",format["A saisie %1 sachet(s) de cocaine | POS : %2 | VEHICULE : %3 | VALEUR : %5 €.", _cf, getPos _v, typeOf _v,[_pf] call ALF_fnc_numberText]] remoteExec ["ALF_Server_fnc_logIt",2];

	private _a = ["Le véhicule a bien été saisie. Voulez-vous publier l'enquête sur BFM TV?","BFM TV","OUI","NON"] call BIS_fnc_guiMessage;
	if !(_a) exitWith {};
	["FLASH INFO",format["Après une lourde enquête, l'auteur transportant une cargaison de cocaine vient d'être arrêté par la Gendarmerie. Le préjudice serait d'une valeur de plus de %1€. Pour des raisons confidentielles, le nom de l'auteur des faits ne sera pas divulgué.",[_pf] call ALF_fnc_numberText]] remoteExec ["ALF_fnc_doBfmO",playableUnits select {alive _x}];
} else {
