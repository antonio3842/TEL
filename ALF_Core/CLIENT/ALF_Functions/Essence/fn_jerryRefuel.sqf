...
switch (true) do {
    case (_vehicle isKindOF "LandVehicle"): {
        if(!local _vehicle) then {
            [_vehicle,(Fuel _vehicle) + 0.5] remoteExecCall ["ALF_fnc_setFuel",_vehicle];
        } else {
    ...
    case (_vehicle isKindOf "Air"): {
        if(!local _vehicle) then {
            [_vehicle,(Fuel _vehicle) + 0.2] remoteExecCall ["ALF_fnc_setFuel",_vehicle];
        } else {
    ...
    case (_vehicle isKindOf "Ship"): {
        if(!local _vehicle) then {
            [_vehicle,(Fuel _vehicle) + 0.35] remoteExecCall ["ALF_fnc_setFuel",_vehicle];
        } else {
