...
switch (_mode) do 
{
	...
	case 0: 
	{
		if (isEngineOn _veh) exitWith {["Essence", "Vous ne pouvez pas poser le pistolet lorsque votre véhicule est allumé", "danger", false] spawn ALF_fnc_doMsg;};
		...
		//Fuel du vehicule
		[_veh,_ActivePistoletObj,player,(getPos player)] remoteExec ["ALF_Server_fnc_fuelVehicule",2];
	};
	...
	case 1: 
	{
		...
		[(_ActivePistoletObj getVariable ["ALF_ActivePompe",objNull]),(_ActivePistoletObj getVariable ["ALF_ActivePistolet","pistolet_1"])] spawn 
		{
			...
			for "_i" from 0 to 1 step 0 do 
			{
				...
				if ((_pistolet isEqualTo "pistolet_1" AND {_dir <= 180 AND {_dir >= 0}}) OR {_pistolet isEqualTo "pistolet_2" AND {_dir <= 360 AND {_dir >= 180}}}) exitWith {[1,_pompe,_pistolet] spawn ALF_fnc_pistoletEssence};
				sleep 0.05;
			};
		};
	};
