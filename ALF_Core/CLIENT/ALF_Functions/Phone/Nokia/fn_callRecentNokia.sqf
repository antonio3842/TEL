...
if(_anonyme) exitWith {hint "Vous ne pouvez rappeler un numéro anonyme.";};

if !(currentWeapon player isEqualTo "") exitWith {["Telephone", "Vous avez les mains encombrés.", "danger", false] spawn ALF_fnc_doMsg;};
if((ALF_Forfait isEqualTo 0) && {!(_numbercalling in ["17","18"])}) exitWith {hint "Votre forfait est épuisé.";};
...
[ALF_Phone_Anonyme,ALF_Phone_Number,_number,player] remoteExec ["ALF_Server_fnc_callSystem",2];
