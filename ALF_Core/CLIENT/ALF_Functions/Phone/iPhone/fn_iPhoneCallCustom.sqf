...
if(_length > 10) exitWith {hint "Vous ne pouvez écrire que 10 caractères max."};
private _badChar = false;
{if(!(_x in _allowed)) exitWith {_badChar = true;};} forEach _chrByte;
if(_badChar) exitWith {hint localize "STR_GNOTF_IncorrectChar";};
if !(currentWeapon player isEqualTo "") exitWith {["Telephone", "Vous avez les mains encombrés.", "danger", false] spawn ALF_fnc_doMsg;};

if(_numbercalling isEqualTo ALF_Phone_Number) exitWith {hint "Vous ne pouvez t'appeler vous même.";};

if(_length < 10 && {!(_numbercalling in ["17","18","124","21"])}) exitWith {hint "Vous ne pouvez écrire que 10 caractères min."};
if((ALF_Forfait isEqualTo 0) && {!(_numbercalling in ["17","18","21"])}) exitWith {hint "Votre forfait est épuisé.";};
...
[ALF_Phone_Anonyme,ALF_Phone_Number,_numbercalling,player] remoteExec ["ALF_Server_fnc_callSystem",2];
...
private _boucle = 5;
if(_numbercalling in ["17","18","124","21"]) then {
	_boucle = 60;
};
