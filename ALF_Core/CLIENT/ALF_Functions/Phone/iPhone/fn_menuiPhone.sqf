// ALF_fnc_menuiPhone = {
if (ALF_Teamspeak_Plugin_Enabled OR {ALF_Teamspeak_Server_Enabled} OR {ALF_Teamspeak_Channel_Enabled}) exitWith {};
...
if (!isNil "ALF_fnc_iPhoneTaxiEnsureControls") then {
	[_display, false] call ALF_fnc_iPhoneTaxiEnsureControls;
};

private _menu1btntaxi = _display displayCtrl 564016;
private _menu11taxi = _display displayCtrl 682570;
private _menu11taxistat = _display displayCtrl 682571;
private _menu11taxicall = _display displayCtrl 682572;
private _menu11taxicallHit = _display displayCtrl 682575;
private _menu11taxicallShadow = _display displayCtrl 682574;
...
private _menu1btntwitter = _display displayCtrl 564015;
...
private _menu5viberON = _display displayCtrl 565058;
private _menu5viberOFF = _display displayCtrl 565059;
...
private _menu5AnonymeON = _display displayCtrl 565063;
private _menu5AnonymeOFF = _display displayCtrl 565062;
private _menu5btnviber = _display displayCtrl 565060;
private _menu5btnavion = _display displayCtrl 565061;
private _menu5avionON = _display displayCtrl 565052;
private _menu5avionOFF = _display displayCtrl 565053;
...
private _menu10twitter = _display displayCtrl 682568;
private _menu10twittergroup = _display displayCtrl 682569;
...
_menu11taxi ctrlShow false;
_menu11taxistat ctrlShow false;
_menu11taxicall ctrlShow false;
if (!isNull _menu11taxicallHit) then { _menu11taxicallHit ctrlShow false; };
if (!isNull _menu11taxicallShadow) then { _menu11taxicallShadow ctrlShow false; };
_menu1btntaxi ctrlShow false;
switch (_menu) do 
{
	case 0 : 
	{
		...
		_menu10twitter ctrlShow false;
		_menu10twittergroup ctrlShow false;
		_menu11taxi ctrlShow false;
		_menu11taxistat ctrlShow false;
		_menu11taxicall ctrlShow false;
		if (!isNull _menu11taxicallHit) then { _menu11taxicallHit ctrlShow false; };
		...
		if (ALF_Phone_inCall) then
		{
			...
			{
				{
					if ((_x select 1) isEqualTo _hisNumber) exitWith {_hisName = _x select 0;};
			...
			if (_hisNumber isEqualTo "17") then {_hisName = "Gendarmerie"} else 
			{
				...
				{
					if (_hisNumber isEqualTo "21") then 
					{
						_hisName = "Police Municipale"
					} else 
					{
		...
		} else 
		{
			if (ALF_Phone_Ring) then 
			{
				...
				_menu1Name ctrlSetStructuredText parseText format["<t align='center' shadow='0' color='#FFFFFF' font='HelveticaLTLight' size='1.5'>%1</t>", _hisName];
				_menu1Num ctrlSetStructuredText parseText format["<t align='center' shadow='0' color='#FFFFFF' font='HelveticaLTLight' size='1.2'>%1</t>", _hisNumber];
				_menu1vert buttonSetAction "player setVariable [""call_info"",2,2];";
			...
			} else 
			{
				if(ALF_Phone_tryCall) then 
				{
					...
					if (_hisNumber isEqualTo "21") then {_hisName = "Police Municipale"};
				...
				} else 
				{
					...
					} else 
					{
						...
						_menu1btntaxi ctrlSetPosition [0.683411 * safezoneW + safezoneX, 0.43923612 * safezoneH + safezoneY, 0.02929688 * safezoneW, 0.04861112 * safezoneH];
						_menu1btntaxi ctrlCommit 0;
						_menu1btntaxi ctrlEnable true;
						_menu1btntaxi ctrlShow true;
						_menu1nosim ctrlShow false;
						_menu1nosimok ctrlShow false;
						_menu1appelentrant ctrlShow false;
						_menu1appel ctrlShow false;
						...
						_menu1btnreglages ctrlSetTextColor [1,0,0,1];
						...
						_menu1btntwitter buttonSetAction "uiNamespace setVariable ['iphonemenu',10]; [] call ALF_fnc_menuiPhone;";
						_menu1btntaxi buttonSetAction "uiNamespace setVariable ['iphonemenu',11]; [] call ALF_fnc_menuiPhone;";
					};
				};
			};
		};
...
_menu3Num ctrlSetStructuredText parseText format["<t align='left' shadow='0' color='#000000' font='HelveticaLTLight' size='1'>%1</t>", _myNumber];
...
_menu3fichename ctrlSetStructuredText parseText format["<t align='left' shadow='0' color='#000000' font='HelveticaLTLight' size='1.5'>%1</t>", _cible select 0];
_menu3fichenum ctrlSetStructuredText parseText format["<t align='left' shadow='0' color='#000000' font='HelveticaLTLight' size='1'>%1</t>", _cible select 1];
...
case 5 : {
	...
	{
		_menu5AnonymeON ctrlShow true; 
		_menu5AnonymeOFF ctrlShow false;
	} else 
	{
		_menu5AnonymeON ctrlShow false; 
		_menu5AnonymeOFF ctrlShow true;
	};
	...
	switch (profileNamespace getVariable ["iPhone_Background",0]) do 
	{
		...
		case 1 : {_text = "Nuit 1";};
		case 2 : {_text = "Nuit 2";};
		case 3 : {_text = "Blup Blup";};
		case 4 : {_text = "Grahouuu";};	
		case 5  : { _text = "Meuf"; };
		case 6  : { _text = "Abstrais 1"; };
		case 7  : { _text = "Abstrais 2"; };
		case 8  : { _text = "OM"; };
		case 9  : { _text = "PSG"; };
		case 10 : { _text = "Mec"; };
		case 11 : { _text = "Bulle"; };
		case 12 : { _text = "Bulle 2"; };
		case 13 : { _text = "Firefox"; };
		case 14 : { _text = "Soldat"; };
		case 15 : { _text = "Jack skell"; };
		case 16 : { _text = "Planète abstraite"; };
	};

	_menu5textbg ctrlSetStructuredText parseText format["<t align='left' shadow='0' color='#005689' font='HelveticaLTLight' size='1'>%1</t>",_text];
	_menu5btnbg buttonSetAction "[] spawn ALF_fnc_setiPhoneBackGround2;";

	switch (profileNamespace getVariable ["iPhone_Ring",0]) do 
	{
		case 0 : {_text = "Standard";};
		case 1 : {_text = "Minions";};
		case 2 : {_text = "Oklm";};
		case 3 : {_text = "Mec";};
		case 4 : {_text = "Meuf";};
	};

	_menu5textring ctrlSetStructuredText parseText format["<t align='left' shadow='0' color='#005689' font='HelveticaLTLight' size='1'>%1</t>",_text];
	...
	{
		_menu5avionON ctrlShow true; 
		_menu5avionOFF ctrlShow false;
	} else 
	{
		_menu5avionON ctrlShow false; 
		_menu5avionOFF ctrlShow true;
	};
...
[ALF_Phone_Number,player] remoteExecCall ["ALF_Server_fnc_loadSmsiPhone",2];
ALF_Phone_SMS = false;
...
_menuRadioText ctrlSetStructuredText parseText format["<t align='left' shadow='0' color='#FF0000' font='HelveticaLTLight' size='1'>Déconnecté</t>"];
_menuRadioVolp ctrlShow false;
_menuRadioVolm ctrlShow false;
...
_menuRadioText ctrlSetStructuredText parseText format["<t align='left' shadow='0' color='#00FF00' font='HelveticaLTLight' size='0.8'>Connected: %1Mhz</t>",_freq];
...
if(_number isEqualTo "21") then {_hisName = "Police Municipale"};
...
_menu10twitter ctrlShow true;
_menu10twittergroup ctrlShow true;
_menu11taxi ctrlShow false;
_menu11taxistat ctrlShow false;
_menu11taxicall ctrlShow false;

{
private _button = _display ctrlCreate ["Life_RscStructuredText",_foreachindex + 100];
_button ctrlSetPosition [safeZoneX + safeZoneW * 0.391875,safeZoneY + safeZoneH * (0.27 + 0.065 * _index), safeZoneW * 0.215,safeZoneH * 0.05444445];
_button ctrlCommit 0;
_button ctrlSetText _displayName;
_button buttonSetAction _statement;			
} forEach ALF_TWITTER_MESSAGES;
};
case 11 : {
_menu1sim ctrlShow false;
_menu1btntel ctrlShow false;
...
// Supprime les boutons Twitter créés dynamiquement (évite superposition)
for "_i" from 0 to 40 do {
	private _ctrl = _display displayCtrl (100 + _i);
	if (!isNull _ctrl) then { ctrlDelete _ctrl; };
};

[] call ALF_fnc_iPhoneTaxiMenu;
private _taxiBg = _display displayCtrl 682570;
if (!isNull _taxiBg) then { _taxiBg ctrlShow true; };
if (!isNull _menu11taxicallShadow) then { _menu11taxicallShadow ctrlShow false; };
};
};
...
[] spawn 
{
	if (isNull ALF_PhoneObject AND {!ALF_Phone_inCall AND {!ALF_Phone_tryCall}}) then 
	{
		...
		waitUntil {(isNull findDisplay 56400) OR ALF_Phone_inCall OR ALF_Phone_tryCall};

		if (!ALF_Phone_inCall AND {!ALF_Phone_tryCall}) then 
		{
