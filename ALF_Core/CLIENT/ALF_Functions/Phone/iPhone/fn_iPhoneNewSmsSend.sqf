...
if(_msg isEqualTo "") exitWith {hint "Votre message est vide.";};
private _length = count (toArray(_number));
private _chrByte = toArray (_number);
private _allowed = toArray("0123456789");
if !(_length isEqualTo 10) exitWith {hint "Vous ne pouvez ecrire que 10 numeros."};
...
if(_length2 > 500) exitWith {hint "Vous ne pouvez ecrire que 500 caractères max."};
{if(!(_x in _allowed2)) exitWith {_badChar = true;};} forEach _chrByte2;
if(_badChar) exitWith {hint localize "STR_GNOTF_IncorrectChar";};

if(ALF_Forfait isEqualTo 0) exitWith {hint "Votre forfait est epuise.";};
...
[_anonyme,ALF_Phone_Number,_number,_msg] remoteExecCall ["ALF_Server_fnc_sendSmsPhone",2];
