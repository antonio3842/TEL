...
if(_datamsg isEqualTo "") exitWith {hint "Vous ne pouvez pas envoyer de message vide."};
...
if(_length > 500) exitWith {hint "Vous ne pouvez ecrire que 500 caractères max."};
private _badChar = false;
{if(!(_x in _allowed)) exitWith {_badChar = true;};} forEach _chrByte;
if(_badChar) exitWith {hint localize "STR_GNOTF_IncorrectChar";};
private _length2 = count (toArray(_number));
if(_length2 < 10) exitWith {hint "Vous ne pouvez pas repondre à ce numero."};
