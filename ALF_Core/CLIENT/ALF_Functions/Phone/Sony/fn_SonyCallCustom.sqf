...
if(_length > 10) exitWith {hint "Vous ne pouvez ecrire que 10 caractères max."};
private _badChar = false;
{if(!(_x in _allowed)) exitWith {_badChar = true;};} forEach _chrByte;
if(_badChar) exitWith {hint localize "STR_GNOTF_IncorrectChar";};

if(_numbercalling isEqualTo ALF_Phone_Number) exitWith {hint "Vous ne pouvez pas t'appeler vous même.";};

if(_length < 10 && {!(_numbercalling in ["17","18","124","21"])}) exitWith {hint "Vous ne pouvez écrire que 10 caractères min."};
if((ALF_Forfait isEqualTo 0) && {!(_numbercalling in ["17","18","21"])}) exitWith {hint "Votre forfait est epuise.";};
