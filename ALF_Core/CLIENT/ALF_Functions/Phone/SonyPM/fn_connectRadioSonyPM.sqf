...
if(_freq isEqualTo 80) then {
	...
	ALF_Sony_FreqAdd = 80;
	player setVariable ["SonyFreqAdd",80,true];
...
[] spawn {
	if !(life_radio_connected) then {
		...
		for "_i" from 0 to 1 step 0 do {
			...
			if !(alive player) exitWith {[_myRadio] call ALF_fnc_sonyPMRadioDisconnect;};
			_radios = player call TFAR_fnc_radiosList;
			uisleep 1;
			if(count _radios < 1) exitWith {[_myRadio] call ALF_fnc_sonyPMRadioDisconnect;};			
			if !([(call TFAR_fnc_activeSwRadio),_myRadio] call TFAR_fnc_isSameRadio) exitWith {[_myRadio] call ALF_fnc_sonyPMRadioDisconnect;};	
			_radios  = [];		
		};
	};
};
