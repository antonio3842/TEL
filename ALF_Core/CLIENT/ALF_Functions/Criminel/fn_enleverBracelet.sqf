...
switch (_mode) do {
	...
	case 0: {
		...
		_titleText ctrlSetText format["Déverrouillage bracelet (1%1)...","%"];
		...
		if(_dice <= 20) then {
			["INFO", "Vous avez enlevé le bracelet électronique de votre collègue.", "success"] spawn ALF_fnc_doMsg;
			...
			[_unit] remoteExecCall ["ALF_Server_fnc_enleverBracelet",2];
			...
			["Bracelet",format["Le signal de %1 vient d'être perdu. La dernière position est indiqué sur la carte.",name _unit]] remoteExecCall ["ALF_Server_fnc_doCopCall",2];
			[name player,getPlayerUID player,"Lockpick",format["A lockpick (WIN) le bracelet de %1.",name _unit]] remoteExec ["ALF_Server_fnc_logIt",2];
		} else {
			["ALF_Lockpick",false] spawn ALF_fnc_handleItem;
			["INFO", "Vous n'avez pas réussi à enlever le bracelet électronique de votre collègue !", "warning"] spawn ALF_fnc_doMsg;
			[name player,getPlayerUID player,"Lockpick",format["A lockpick (FAIL) le bracelet de %1.",name _unit]] remoteExec ["ALF_Server_fnc_logIt",2];
			sleep 120;
	...
	case 1: {
		if !(player canAdd "ALF_Bracelet") exitWith {["INFO", "Vous n'avez pas assez de place pour récupérer le bracelet électronique !", "warning"] spawn ALF_fnc_doMsg;};
		["ALF_Bracelet",true] spawn ALF_fnc_handleItem;
		_array = missionNamespace getVariable ["ALF_AllBracelets",[]];
		_index = _array find _unit;
		if (_index > -1) then {
		...
		["INFO", "Vous avez enlevé le bracelet électronique de la personne.", "success"] spawn ALF_fnc_doMsg;
		[_unit] remoteExecCall ["ALF_Server_fnc_enleverBracelet",2];
	};
};
