...
if(!_isVehicle) then {
    ...
    [name player,getPlayerUID player,"Lockpick",format["A lockpick les menottes de %1.",name _curTarget]] remoteExec ["ALF_Server_fnc_logIt",2];
} else {
    private _dice = random(1000);

    private _veh = _curTarget; // si c'est un véhicule
    private _plaque = _veh getVariable ["plate","Inconnue"];
    private _nomVeh = getText(configFile >> "CfgVehicles" >> typeOf _veh >> "displayName");

    //private _plaque = _curTarget getVariable ["plate","Inconnue"];
    //private _nomVeh = getText(configFile >> "CfgVehicles" >> (typeOf _curTarget) >> "displayName");
    private _adn = player getVariable "adnface" select 0;
    if(_dice <= 200) then {
        ["INFO", "Vous avez ouvert le véhicule.", "warning"] spawn ALF_fnc_doMsg;

        [name player,getPlayerUID player,"Lockpick",format["A lockpick (SUCCÈS sans alarme) %1 (Plaque: %2)",_nomVeh,_plaque]] remoteExec ["ALF_Server_fnc_logIt",2];

        //[name player,getPlayerUID player,"Lockpick",format["A lockpick (WIN sans alarme) %1",getText(configFile >> "CfgVehicles" >> (typeOf _curTarget) >> "displayName")]] remoteExec ["ALF_Server_fnc_logIt",2];
        if(local _curTarget) then {
            _curTarget lock 0;
        } else {
            [_curTarget,0] remoteExecCall ["ALF_fnc_lockVehicle",_curTarget];
        };
    ...
    if(_dice > 200 && {_dice < 500}) then {
        ["INFO", "Vous avez ouvert le véhicule mais vous avez déclenché l'alarme du véhicule.", "warning"] spawn ALF_fnc_doMsg;

        [name player,getPlayerUID player,"Lockpick",format["A lockpick (SUCCÈS avec alarme) %1 (Plaque: %2)",_nomVeh,_plaque]] remoteExec ["ALF_Server_fnc_logIt",2];

        //[name player,getPlayerUID player,"Lockpick",format["A lockpick (WIN avec alarme) %1.",getText(configFile >> "CfgVehicles" >> (typeOf _curTarget) >> "displayName")]] remoteExec ["ALF_Server_fnc_logIt",2];
        [_curTarget,"ALF_CarAlarm1"] remoteExec ["ALF_fnc_say3D",RANY];
        if(local _curTarget) then {
            _curTarget lock 0;
        } else {
            [_curTarget,0] remoteExecCall ["ALF_fnc_lockVehicle",_curTarget];
    ...
    if(_dice >= 500) then {
        ["INFO", "Vous avez cassé le lockpick !", "warning"] spawn ALF_fnc_doMsg;

        [name player,getPlayerUID player,"Lockpick",format["A lockpick (ÉCHEC) %1 (Plaque: %2)",_nomVeh,_plaque]] remoteExec ["ALF_Server_fnc_logIt",2];

        //[name player,getPlayerUID player,"Lockpick",format["A lockpick (FAIL) %1.",getText(configFile >> "CfgVehicles" >> (typeOf _curTarget) >> "displayName")]] remoteExec ["ALF_Server_fnc_logIt",2];
