...
// |		Scripts : fn_braquageStation.sqf |
// |______________________________________ */

//ALF_fnc_braquageStation = {
...
{
	["Braquage", "Vous avez crus voir un gendarme, vaut mieux remettre ca à plus tard", "warning", false] spawn alf_fnc_doMsg;
};

// private _heure_Serveur = systemTime # 3;
// if ((_heure_Serveur < 23) AND {_heure_Serveur >= 3}) exitwith 
// {
// 	["Braquage", "Vous avez l'impression qu'on vous observe, vaut mieux remettre ca à dans la soirée", "warning", false] spawn alf_fnc_doMsg;
// };
...
private _ville_Proche = text ((nearestLocations [position _mec_Braquer, ["NameLocal","NameVillage","NameCity","NameCityCapital"], 1500]) # 0);

private _bfm_sent_25 = false;
private _bfm_sent_50 = false;
private _bfm_sent_70 = false;
...
for "_i" from 0 to 1 step 0 do
{
	...
	if (!_bfm_sent_25 && _cP >= 0.25 && _alerte) then {
		_bfm_sent_25 = true;
	...
	if (!_bfm_sent_50 && _cP >= 0.50 && _alerte) then {
		_bfm_sent_50 = true;
		["Braquage", format["On a aperçu les braqueurs près de %1 ! Prudence aux alentours !", _ville_Proche]] remoteExec ["ALF_fnc_doBfmO", playableUnits];
	};
	if (!_bfm_sent_70 && _cP >= 0.70 && _alerte) then {
		_bfm_sent_70 = true;
		["Braquage", format["On a aperçu les braqueurs près de %1 ! Prudence aux alentours !", _ville_Proche]] remoteExec ["ALF_fnc_doBfmO", playableUnits];
	};
	if (_alerte) then {
...
_toAdd = round(random [15000, 30000, 60000]);
