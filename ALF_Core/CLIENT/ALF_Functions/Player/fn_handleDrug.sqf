...
if (alf_drug isEqualTo 0) then {
	...
	[] spawn {
		...
		while {alf_drug > 0} do {
			...
			waitUntil {ppEffectCommitted _effect};
			alf_drug = alf_drug - _reduce;
			// S'assurer que alf_drug ne reste pas bloqué à une valeur très petite
			if (alf_drug <= 0.005) then {alf_drug = 0;};
		};

		// Garantir que alf_drug est exactement à 0
		alf_drug = 0;
		[13] spawn SOCK_fnc_updatePartial;
		resetCamShake;
