...
if(!isNull _source) then {
	if !(_source isEqualTo _unit) then {
		if (currentWeapon _source in ["MRP_X26"] && {_projectile in ["MRP_Taser_Ammo"]}) then {
				...
				if (_unit distance _source < 40) then {
					if !(isNull objectParent player) then {
						if (typeOf (vehicle player) in ["B_Quadbike_01_F"]) then {
							...
							[_unit,_source] spawn ALF_fnc_tazed;
						};
					} else {
						[_unit,_source] spawn ALF_fnc_tazed;
					};
		...
		if (currentWeapon _source in ["MRP_LBD40"] && {_projectile in ["MRP_LBD_Ammo"]}) then {
			_damage = getDammage player;
			if (alive player && {!life_istazed} && {!life_isknocked}) then {
				if (_unit distance _source < 60) then {
					if !(isNull objectParent player) then {
						if (typeOf (vehicle player) in ["B_Quadbike_01_F"]) then {
							player action ["Eject",vehicle player];
							[_unit] spawn ALF_fnc_knockedOut;
						};
					} else {
						[_unit] spawn ALF_fnc_knockedOut;
					};
