...
if(_dice <= 25) then {
	["INFO", "Vous avez coupé le barbelé.", "success"] spawn ALF_fnc_doMsg;
	[name player,getPlayerUID player,"PRISON","A COUPER (REUSSIT) LE BARBELE."] remoteExec ["ALF_Server_fnc_logIt",2];
	...
	if(_dice >= 90) then {
		["INFO", "Vous avez cassé votre paire de ciseaux inox.", "warning"] spawn ALF_fnc_doMsg;
		["ALF_ZipCutterP",false] call ALF_fnc_handleItem;
	} else {
		["INFO", "Vous n'avez pas réussit à couper le barbelé.", "warning"] spawn ALF_fnc_doMsg;
	};
	[name player,getPlayerUID player,"PRISON","A COUPER (ECHEC) LE BARBELE."] remoteExec ["ALF_Server_fnc_logIt",2];
};
