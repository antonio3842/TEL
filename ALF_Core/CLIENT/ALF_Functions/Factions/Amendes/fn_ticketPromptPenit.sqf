...
*/
private["_cop","_val"];
if(!isNull (findDisplay 0505201)) exitWith {}; //Already at the ticket menu, block for abuse?
_cop = _this select 0;
if(isNull _cop) exitWith {};
_val = _this select 1;

createDialog "life_ticket_payPenit";
disableSerialization;
waitUntil {sleep 0.3; !isNull (findDisplay 0505201)};

life_ticket_paid = false;
life_ticket_val = _val;
life_ticket_cop = _cop;
CONTROL(0505201,2601) ctrlSetStructuredText parseText format["<t align='center'><t size='.8px'>Amende de %1€.",_val];

[] spawn {
	disableSerialization;
	waitUntil {sleep 0.3; life_ticket_paid OR {isNull (findDisplay 2600)}};
	if(isNull (findDisplay 0505201) && {!life_ticket_paid}) then {
