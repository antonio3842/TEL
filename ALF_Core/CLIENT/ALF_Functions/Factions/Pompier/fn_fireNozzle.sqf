/*
    fireNozzle
    Nanou
    Restaure a l'ancien systeme - deleteVehicle direct
*/
private _p = _this select 0;
private _nearFires = nearestObjects [_p, ["test_EmptyObjectForFireBig","ALF_GrosFeuForet","ALF_MoyenFeuForet"], 7];

if (_nearFires isEqualTo []) exitWith {};

private _f = _nearFires select 0;

if(isNull _f) exitWith {};
if(_p distance _f > 8) exitWith {};
...
if(_v isEqualTo 0) then {
    ...
    if (_typeof isEqualTo "test_EmptyObjectForFireBig") then {
            ...
            if(count _h > 0) then {
                {
                    ...
                    if(_x getVariable ["trainingfire",false]) then {
                        ...
                        ["BRAVO","EXERCICE INCENDIE TERMINE FELICITATION !!"] remoteExecCall ["ALF_Server_fnc_doMedCall",2];
                    };
                } forEach _h;
            };
        };
        ...
        if (_typeof in ["ALF_GrosFeuForet","ALF_MoyenFeuForet"]) then {
            ...
            if( (count _c) isEqualTo 0 ) then {
                ["BRAVO","VOUS AVEZ ETEINT L'INCENDIE DE FORET FELICITATION !!"] remoteExecCall ["ALF_Server_fnc_doMedCall",2];
    ...
    if (_v <= 30 && {_typeof isEqualTo "ALF_GrosFeuForet"}) then {
        // Remplacer GrosFeu par MoyenFeu directement
        private _pos = getPosATL _f;
        deleteVehicle _f;
        waitUntil {isNull _f};
        
        private _feu = "ALF_MoyenFeuForet" createVehicle [0,0,0];
        _feu setPosATL _pos;
        _feu setVariable ["firealf", _v, true];
    } else {
