...
if !(isNull _atmNearest) then {
	_cursorObject = _atmNearest;
};
private _pAtm = getPosWorld _cursorObject;
...
if !(createDialog "ALF_Laptop_Dialog") exitWith {
	["INFO", "Impossible d'ouvrir le menu.", "warning"] spawn ALF_fnc_doMsg;
};
disableSerialization;
