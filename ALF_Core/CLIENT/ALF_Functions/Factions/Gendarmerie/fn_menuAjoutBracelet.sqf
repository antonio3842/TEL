...
switch (_mode) do {
	...
	case 1: {
		...
		["ALF_Bracelet",false] spawn ALF_fnc_handleItem;
		closeDialog 0;
		...
		if (_minutesTotal isEqualTo 0) then {
			[-10,ALF_unitAjoutBracelet] remoteExecCall ["ALF_Server_fnc_braceletElectronique",2];
		} else {
			[_minutesTotal,ALF_unitAjoutBracelet] remoteExecCall ["ALF_Server_fnc_braceletElectronique",2];
		};
	};
};
