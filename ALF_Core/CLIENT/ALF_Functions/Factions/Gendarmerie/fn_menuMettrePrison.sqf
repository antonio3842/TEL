...
switch (_mode) do {
	...
	case 1: {
		...
		[_minutesTotal,ALF_unitMettrePrison] remoteExecCall ["ALF_Server_fnc_mettrePrison",2];
	};
	...
	case 2: {
		...
		if (player in _array) then {
			...
			[player] remoteExecCall ["ALF_Server_fnc_enleverBracelet",2];
		};
	...
	case 5: {
		...
		if (_cellule > 10 || _cellule < 1) exitWith {["INFO","Vous devez indiquer un nombre entre 1 et 10.","warning"] spawn ALF_fnc_doMsg;};

		closeDialog 0;
		["INFO",format ["Vous venez d'assigner la cellule %1 pour %2.",_cellule, name ALF_unitMettrePrison],"success"] spawn ALF_fnc_doMsg;

		//On envoie a l'HC
		[_cellule,ALF_unitMettrePrison] remoteExecCall ["ALF_Server_fnc_mettreCellule",2];
	};
	case 6: {
