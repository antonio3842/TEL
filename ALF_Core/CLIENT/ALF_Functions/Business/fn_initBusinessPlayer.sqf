...
switch (_mode) do {
	case 0 : {
		switch (_secteur) do {
			...
			case 28 : {
				b_28 = true; //TLA (Avocat)
			};
			case 29 : {
				b_29 = true; //Entreprise de minage de fer "Fer'ever"
			};
			case 30 : {
				b_30 = true; //Jus raisin
			};
			case 31 : {
				b_31 = true; //Jus fraise
			};
			case 32 : {
				b_32 = true; //Café
			};
			case 33 : {
				b_33 = true; //MMPrestige
				player setVariable ["b_33",true,true]; //Helios
			};
			case 34 : {
				b_34 = true; //l'association de plongé
			};
			case 35 : {
				b_35 = true; //Maison médical
			};
		};

		player setVariable ["isEmployed",_siret];
		player setVariable ["TelPro",_tel,true];
		// player setVariable ["MRPV2_Emploiement_Joueur",[_siret,]];
		private _arr = missionNamespace getVariable [format["%1_KEYSB",_siret],[]];
		{life_vehicles pushBack _x} forEach _arr;

		{if((_x # 1) isEqualTo (getPlayerUID player)) then {alf_b_level = (_x # 2)}} forEach _members;
	};
	case 1 : {
		switch (_secteur) do {
			case 0 : {
		...
		["INFO","Vous avez été viré par votre employeur.","warning"] spawn ALF_fnc_doMsg;
	};
