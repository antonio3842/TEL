...
switch (_mode) do {
  ...
  case 0: {
    ...
    [_siret] remoteExecCall ["ALF_Server_fnc_fiscToPlayer", 2];
  };
  ...
  case 2: {
    ...
    if !(createDialog "ALF_FISC_Dialog") exitWith {["INFO","Une erreur s'est produite lors de l'ouverture des comptes de l'entreprise.","warning"] spawn ALF_fnc_doMsg;};
    ...
    _control ctrlSetStructuredText (parseText (format ["<t size='0.8' align='center' color='#000000'>Le capital de l'entreprise est de %1€</t>",[(ALF_Fisc_Building getVariable ["business",[1,1,1,1,1,1,1,1,"Erreur"]]) select 3] call ALF_fnc_numberText]));
  };
  ...
  case 5: {
    ...
    if (_modev isEqualTo 0) then {
      ...
      {
        private _index = lbAdd [18543, [_x select 4] call ALF_fnc_DateToString];
    ...
    switch (lbValue [18543,_index]) do {
      case 0: {
        private _text = [];
        {
          if ((_x select 2) isEqualTo 1) then {
            _text pushBack (format ["<t size='0.75' color='#008E07'>%1 a reçu %2€ par l'entreprise le %3 et a été déclaré.</t><br />",[_x select 0,ALF_Fisc_Building getVariable ["business",[1,1,1,1,1,1,1,1,"Erreur"]] select 8] select ((_x select 0) isEqualTo "Moi"),[(_x select 1)] call ALF_fnc_numberText,[_x select 4] call ALF_fnc_DateToString]);
          } else {
            _text pushBack (format ["<t size='0.75' color='#DE0A01'>%1 a reçu %2€ par l'entreprise le %3 et n'a pas été déclaré.</t><br />",[_x select 0,ALF_Fisc_Building getVariable ["business",[1,1,1,1,1,1,1,1,"Erreur"]] select 8] select ((_x select 0) isEqualTo "Moi"),[(_x select 1)] call ALF_fnc_numberText,[_x select 4] call ALF_fnc_DateToString]);
          };
        } forEach _data;
      ...
      case 1: {
        ...
        if ((_data select 2) isEqualTo 1) then {
          _text = format ["<t size='0.75' color='#008E07'>%1 a reçu %2€ par l'entreprise le %3 et a été déclaré</t><br />",[_data select 0,ALF_Fisc_Building getVariable ["business",[1,1,1,1,1,1,1,1,"Erreur"]] select 8] select ((_data select 0) isEqualTo "Moi"),[(_data select 1)] call ALF_fnc_numberText,[_data select 4] call ALF_fnc_DateToString];
        } else {
          _text = format ["<t size='0.75' color='#DE0A01'>%1 a reçu %2€ par l'entreprise le %3 et n'a pas été déclaré</t><br />",[_data select 0,ALF_Fisc_Building getVariable ["business",[1,1,1,1,1,1,1,1,"Erreur"]] select 8] select ((_data select 0) isEqualTo "Moi"),[(_data select 1)] call ALF_fnc_numberText,[_data select 4] call ALF_fnc_DateToString];
        };
        _control ctrlSetStructuredText (parseText _text);
        private _h = ctrlTextHeight _control;
        private _position = ctrlPosition _control;
      ...
      case 2: {
        private _text = [];
        {
          _text pushBack (format ["<t size='0.75' color='#000000'>%1 a payé %2€ à l'entreprise le %3</t><br />",[_x select 0,ALF_Fisc_Building getVariable ["business",[1,1,1,1,1,1,1,1,"Erreur"]] select 8] select ((_x select 0) isEqualTo "Moi"),[(_x select 1)] call ALF_fnc_numberText,[_x select 4] call ALF_fnc_DateToString]);
        } forEach _data;
        _text = _text joinString "";
        _control ctrlSetStructuredText (parseText _text);
        private _h = ctrlTextHeight _control;
      ...
      case 3: {
        private _text = format ["<t size='0.75' color='#000000'>%1 a payé %2€ à l'entreprise le %3</t>",[_data select 0,ALF_Fisc_Building getVariable ["business",[1,1,1,1,1,1,1,1,"Erreur"]] select 8] select ((_data select 0) isEqualTo "Moi"),[(_data select 1)] call ALF_fnc_numberText,[_data select 4] call ALF_fnc_DateToString];
        _control ctrlSetStructuredText (parseText _text);
        private _h = ctrlTextHeight _control;
        private _position = ctrlPosition _control;
        _control ctrlSetPosition [_position select 0,_position select 1,_position select 2,_h];
