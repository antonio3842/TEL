...
// Parachute : addBackpack obligatoire (addItem = invisible / non équipé)
if (_item in ["B_Parachute","MRP_B_Parachute"]) exitWith {
	private _items = nil;
	if (_bool) then {
		...
		if (!isNil "_items") then {
			{ [_x, true, true, false, true] spawn ALF_fnc_handleItem; } forEach _items;
		};
	} else {
		if (backpack player isEqualTo _item) then { removeBackpack player; };
	};
};

private _details = [_item] call ALF_fnc_fetchCfgDetails;
				...
				switch(SEL(_details,5)) do {
						...
						if(_ispack) then {
							...
							if(_override) then{
								...
								if(EQUAL(vest player,_item) && {!_preview}) then {
									...
									if(!isNil {_items}) then {
										{[_x,true,false,false,true] spawn ALF_fnc_handleItem;} forEach _items;
									};
								};
							};
						};
					...
					case 201: {
						if(_ispack) then {
							...
							private "_type";
							_type = [_item,201] call ALF_fnc_accType;
							if(_ongun) then {
								switch (_type) do {
					...
					case 301: {
						if(_ispack) then {
							...
							private "_type";
							_type = [_item,301] call ALF_fnc_accType;
					...
					case 101:{
						if(_ispack) then {
							...
							private "_type";
							_type = [_item,101] call ALF_fnc_accType;
