...
private _item = ["STRING", ["ALF_OBJECTS", typeOf(_object), "item"]] call ALFTools_Client_Config_fnc_getConfig;

private _condition = ["STRING", ["ALF_OBJECTS", typeOf(_object), "conditionRecup"]] call ALFTools_Client_Config_fnc_getConfig;

if !(_condition isEqualTo "") then {
	if (call compile format["%1",_condition]) exitWith {};
};

if (!(player canAdd _item)) exitWith {["INFO", "Vous n'avez pas assez de place.", "danger"] spawn ALF_fnc_doMsg;};
