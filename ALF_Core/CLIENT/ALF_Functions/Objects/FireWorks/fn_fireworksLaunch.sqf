...
private _direction = [_this,13,[],[[]]] call BIS_fnc_param;

private _nul1 ="CMflare_Chaff_Ammo" createVehicle _firing_position; 
private _inten = getpos player vectorMultiply 180;

_nul1 setVelocity [0,0,250]; 
...
sleep 0.10;
_nul2 say3D [format["ALF_%1",_bangSound],450];
_nul2 say3D [format["ALF_%1",_longuedistance],600];

if (player distance _nul2 < 250) then {
	_snd = selectrandom ["ALF_firework_1","ALF_firework_2","ALF_firework_3"];
	playsound _snd;
};

for [{_i=0},{_i < ((count _explosion_fragments_array) min 25)},{_i=_i+1}] do {
