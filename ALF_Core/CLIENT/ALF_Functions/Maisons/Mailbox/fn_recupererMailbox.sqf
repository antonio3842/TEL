...
Author: ALF Team
*/
if((time - life_action_delay) < 0.3) exitWith {["INFO","Vous appuyez trop vite.","warning"] spawn ALF_fnc_doMsg;};
...
if(_num > _value) exitWith {["INFO","Vous n'avez pas autant dans votre boite aux lettres.","warning"] spawn ALF_fnc_doMsg;};
if !(player canAdd [_ctrl,_num]) exitWith {["INFO","Vous n'avez pas assez de places.","warning"] spawn ALF_fnc_doMsg;};
...
for "_i" from 1 to _num step 1 do {
	[_ctrl,true] spawn ALF_fnc_handleItem;
};
...
if(count _courrier isEqualTo 0) then {
	...
	lbClear _maillist;
	{
		_itemInfo = [_x select 0] call ALF_fnc_fetchCfgDetails;
