...
if(_num > _countItem) exitWith {["INFO","Il n'y a pas autant dans votre coffre.","warning"] spawn ALF_fnc_doMsg;};

//ON RECUPERE LES DETAILS DE L'ITEM
private _info = [_item] call ALF_fnc_fetchCfgDetails;
...
if (!(player canAdd [_item,_num]) && {!(_isBackPack)} && {!(_isUniform)} && {!(_isVest)}) exitWith {["INFO","Vous n'avez pas assez de places.","warning"] spawn ALF_fnc_doMsg;};
if(_isBackPack && {_num > 1}) exitWith {["INFO","Tu peux pas ajouter plusieurs sacs en même temps.","warning"] spawn ALF_fnc_doMsg;};
if(_isUniform && {_num > 1}) exitWith {["INFO","Tu peux pas ajouter plusieurs uniformes en même temps.","warning"] spawn ALF_fnc_doMsg;};
if(_isVest && {_num > 1}) exitWith {["INFO","Tu peux pas ajouter plusieurs vestes en même temps.","warning"] spawn ALF_fnc_doMsg;};

//TOUT EST BON, ON LANCE LE TAKEINMOBILIER
...
for "_i" from 1 to _num step 1 do {
    ...
    [_item,true] call ALF_fnc_handleItem;
};
...
[_container] spawn {
    ...
    if !(alf_mobiliersave) then {
        ...
        if !(isNull _container) then {
            [_container] remoteExec ["ALF_Server_fnc_updateMobilierInv",2];
        };
...
private _listedItems = [];
{
    if(!(_x in _listedItems) && {_x != ""}) then {
        private _itemInfo = [_x] call ALF_fnc_fetchCfgDetails;
...
_listedItems = [];
{
    _mass = _mass + (([_x] call ALF_fnc_fetchCfgDetails) select 15);
    if(!(_x in _listedItems) && {_x != ""}) then {
        private _itemInfo = [_x] call ALF_fnc_fetchCfgDetails;
        _listedItems pushBack _x;
...
_massText ctrlSetText format["Poids: %1 / %2 kg",_mass,_coffre];
