...
if(_num > _countItem) exitWith {["INFO","Il n'y a pas autant sur vous.","warning"] spawn ALF_fnc_doMsg;};
...
private _massItem = ([_item] call ALF_fnc_fetchCfgDetails) select 15;
private _mass = 0;
{_mass = _mass + (([_x] call ALF_fnc_fetchCfgDetails) select 15);} forEach alf_craft_table;
...
_massText ctrlSetText format["Poids: %1 / %2 kg",(_mass + (_massItem * _num)),_coffre];
