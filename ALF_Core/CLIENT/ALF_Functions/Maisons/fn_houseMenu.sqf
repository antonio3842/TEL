...
private _houseCfg = [(typeOf _curTarget)] call ALF_fnc_houseConfig;
...
ALF_House_Data = [];
[_curTarget] remoteExecCall ["ALF_Server_fnc_recupHouseData",2];
...
if(count(_house_owner) isEqualTo 0) then 
{
	...
	_Btn1 buttonSetAction "[life_pInact_curTarget] spawn ALF_fnc_buyHouse;";
	_Btn1 ctrlShow true;
...
} else 
{
	...
	{		
		if ((_house_owner select 0) isEqualTo (getPlayerUID player)) then 
		{
			...
			_Btn1 buttonSetAction "[life_pInact_curTarget] spawn ALF_fnc_sellHouse; closeDialog 0;";
			_Btn1 ctrlShow true;
			_Btn1 ctrlEnable true;
		};
		...
		_Btn3 buttonSetAction "[life_pInact_curTarget] call ALF_fnc_lightHouseAction; closeDialog 0;";
		_Btn3 ctrlShow true;
		...
		if ((typeOf _curTarget) in ["Land_ALF_Caravane"] && {"ALF_Barrel_boite" in (magazines player)} && {(_house_owner select 0) isEqualTo (getPlayerUID player)}) then 
		{
			...
			_Btn5 buttonSetAction "[life_pInact_curTarget,player] remoteExecCall [""ALF_Server_fnc_poserBaril"",2]; closeDialog 0;";
			_Btn5 ctrlShow true;
		};

		if((typeOf _curTarget) in ["Land_i_Garage_V1_F","Land_i_Garage_V2_F","Land_Garage","Land_Garage2"]) then 
		{
			_Btn5 ctrlSetText "ACCEDER AU GARAGE";
			_Btn5 buttonSetAction "[life_pInact_curTarget,""Car""] spawn ALF_fnc_vehicleGarage; closeDialog 0;";
			_Btn5 ctrlShow true;

			_Btn6 ctrlSetText "RANGER VEHICULE";
			_Btn6 buttonSetAction "[player,'CAR'] spawn ALF_fnc_storeVehicle; closeDialog 0;";
			_Btn6 ctrlShow true;
		};
		...
		if("ALF_Sonnette4G" in (magazines player) && {_sonnette isEqualTo 0} && {(_house_owner select 0) isEqualTo (getPlayerUID player)}) then 
		{
			...
			_Btn12 buttonSetAction "[life_pInact_curTarget,player] remoteExecCall [""ALF_Server_fnc_sonnetteHouse"",2]; closeDialog 0;";
		...
		if (_sonnette isEqualTo 1) then 
		{
			...
			_Btn2 buttonSetAction "
				...
				[life_pInact_curTarget] remoteExecCall [""ALF_Server_fnc_playSonnetteHouse"",2];
				closeDialog 0;
			";
			_Btn2 ctrlShow true;
		};
...
if(life_gendarme_service) then 
{
	if(count(_house_owner) > 0) then 
	{
		...
		_Btn7 buttonSetAction "[] call ALF_fnc_copHouseOwner;";
		_Btn7 ctrlShow true;

		_Btn8 ctrlSetText "[G] ENFONCER LA PORTE";
		_Btn8 buttonSetAction "[life_pInact_curTarget,0] spawn ALF_fnc_copBreakDoor; closeDialog 0;";
		_Btn8 ctrlShow true;

		_Btn10 ctrlSetText "[G] VERROUILLER MAISON";
		_Btn10 buttonSetAction "[life_pInact_curTarget] spawn ALF_fnc_lockupHouse; closeDialog 0;";
