...
// 				[_container] remoteExec ["ALF_Server_fnc_saveCoffre",2];
// 			};
// 		};
// 	};
// };

if(!isnil {alf_banque_sang} AND {_container isEqualTo alf_banque_sang}) exitWith {
	[_container] spawn {
		_container = _this select 0;
		life_query_time = time;
		if !(alf_gearsave) then {
			...
			alf_gearsave = false;
			if !(isNull _container) then {
				[_container] remoteExec ["ALF_Server_fnc_saveBanqueSang",2];
			};
		};
	};
};
	...
	if !(alf_gearsave) then {
		...
		if !(isNull _veh) then {
			if(_veh isKindOf "Car" OR {_veh isKindOf "Air"} OR {_veh isKindOf "Ship"}) then {
				[_veh,1] remoteExec ["ALF_Server_fnc_vehicleUpdate",2];
			};
		};
	};
};
