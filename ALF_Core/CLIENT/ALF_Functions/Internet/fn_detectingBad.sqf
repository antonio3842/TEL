...
if (_m) then {
	private _ville_Proche = text ((nearestLocations [position player, ["NameLocal","NameVillage","NameCity","NameCityCapital"], 1000]) # 0);
	["Cyber Cafe",format["Notre pare-feu vient de détecter une intrusion sur un site classé dangereux vers %1.",_ville_Proche]] remoteExecCall ["ALF_Server_fnc_doCopCall",2];
...
[_atm,_time,_nearPlayersFace] remoteExecCall ["ALF_Server_fnc_setCameraVar",2];
