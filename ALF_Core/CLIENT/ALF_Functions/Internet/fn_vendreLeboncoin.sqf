...
private _l = count _c;
if (_l > 30) exitWith {hint "Vous ne pouvez ecrire que 30 caractères en titre."};
...
private _l = count _c;
if (_l > 400) exitWith {hint "Vous ne pouvez ecrire que 400 caractères en description."};
private _a = toArray("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ.,'-/[]éàç€!?;:êè)(=+&<>*$ ");
{if(!(_x in _a)) exitWith {_exit = true;};} forEach _c;
...
private _l = count _c;
if (_l > 20) exitWith {hint "Vous ne pouvez ecrire que 20 caractères en nom."};
...
private _action = ["Etes-vous certain de vouloir vendre ce véhicule/maison. La vente n'a pas de retour possible.","Leboncoin","OUI","NON"] call BIS_fnc_guiMessage;
...
if(count _data isEqualTo 3) then {
	...
	[_house,_txt1,_txt2,_txt3,_num,_myNumber,_uid,player] remoteExec ["ALF_Server_fnc_createHouseLeboncoin",2];
} else {
	//if((_data select 1) in ["V_ALF_1400GTR","V_ALF_Scania_P"]) exitWith {["Leboncoin","Vous ne pouvez pas vendre ce type de véhicule.","warning"] spawn ALF_fnc_doMsg;};
	[_data select 0,_data select 1,_txt1,_txt2,_txt3,_num,_myNumber,_uid,player] remoteExec ["ALF_Server_fnc_createVehiculeLeboncoin",2];
