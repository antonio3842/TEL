	...
	if (isNull life_preview_3D_vehicle_object || {_classView != typeOf life_preview_3D_vehicle_object}) then
	{
		...
		if (!life_pos_exist) then {
			if(_classView in _boat) then {
				...
				life_pos_attach = [10087.801,10079.544,0.0];//life_pos_attach = [5585.756,4106.749,0.0];
			};
			life_pos_exist = true;
		...
		if(_classView in _boat) then {
			...
			_object setPosATL [10087.801,10079.544,0.0];//_object setPosATL [5585.756,4106.749,0.0];
		};
