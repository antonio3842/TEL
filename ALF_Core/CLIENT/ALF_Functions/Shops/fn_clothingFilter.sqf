...
private _array = switch(_selection) do {
	case 0: {["ARRAY", ["Clothing", life_clothing_store, "uniforms"]] call ALFTools_Client_Config_fnc_getConfig;};
	case 1: {["ARRAY", ["Clothing", life_clothing_store, "headgear"]] call ALFTools_Client_Config_fnc_getConfig;};
	case 2: {["ARRAY", ["Clothing", life_clothing_store, "goggles"]] call ALFTools_Client_Config_fnc_getConfig;};
	case 3: {["ARRAY", ["Clothing", life_clothing_store, "vests"]] call ALFTools_Client_Config_fnc_getConfig;};
	case 4: {["ARRAY", ["Clothing", life_clothing_store, "backpacks"]] call ALFTools_Client_Config_fnc_getConfig;};
};

{
	...
	if(!(EQUAL(_className,"NONE"))) then {
		_details = [_className] call ALF_fnc_fetchCfgDetails;
		_pic = SEL(_details,2);
	};
