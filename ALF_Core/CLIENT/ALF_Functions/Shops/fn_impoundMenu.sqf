...
// ['ALF_fnc_impoundMenu OSOLETTEEEEEEEEEEE'] remoteExec ['systemchat',0]; // PERF FIX: Debug désactivé
if(!dialog) then 
{
...
for "_i" from 0 to (count life_vehicles)-1 do 
{
	...
	if(_veh distance impound_obj < 50) then 
	{
		...
		if(_price > life_cash) then	{_price = _price + 200;};
		...
		_cars lbAdd format["%1 %2 - Cost: $%3",_name,_text,[_price] call ALF_fnc_numberText];
