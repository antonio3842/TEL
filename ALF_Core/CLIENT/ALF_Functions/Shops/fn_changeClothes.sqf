...
if(_data isEqualTo "NONE") then {
	...
	if (life_clothing_filter isequalto 1) then 
	{
		if (hmd player != "") then 
		{
			player removeWeapon (hmd player);
		};
	};
	
	[_item,false] spawn ALF_fnc_handleItem;
} else {
	[_data,true,nil,nil,nil,nil,nil,true] spawn ALF_fnc_handleItem;
};
...
_price ctrlSetStructuredText parseText format ["Unité : <t color='#8cff9b'>%1 €</t>" + " <t color='#8cff9b'>Total :%2 €</t>",[(_control lbValue _selection)] call ALF_fnc_numberText,[_totalPrice] call ALF_fnc_numberText];
