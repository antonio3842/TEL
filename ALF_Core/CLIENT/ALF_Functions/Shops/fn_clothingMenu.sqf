...
_returnClass = [["Clothing", (_this select 3)]] call ALFTools_Client_Config_fnc_isClass;
if(!_returnClass) exitWith {};

private _license = (["STRING", ["Clothing", (_this select 3), "license"]]) call ALFTools_Client_Config_fnc_getConfig;

private _exit = false;
if(!(EQUAL(_license,""))) then {

	private _varnameConfig = ["STRING", ["Licenses", _license, "variable"]] call ALFTools_Client_Config_fnc_getConfig;
	private _levelConfigVal = missionNamespace getVariable [format["license_%1",_varnameConfig], false];

	if(!_levelConfigVal) exitWith {hint localize "STR_Shop_Veh_NoLicense"; _exit = true;};
};
if(_exit) exitWith {};
...
if((_this select 3) isEqualTo "metal" && {!((getPlayerUID player) isEqualTo "76561198011602582")}) exitWith {hint "Vous n'avez pas le droit."; closeDialog 0;};
if((_this select 3) in ["cop"] && life_copLevel < 1) exitWith {hint localize "STR_Shop_NotaCop"; closeDialog 0;};

private _pos = [10079.544,10047.549,0.0];//private _pos = [5585.756,4106.749,0.0];
...
_returnClass = [["Licenses", life_clothing_store]] call ALFTools_Client_Config_fnc_isClass;
if(_returnClass) then {
	private _displayName = ["STRING", ["Licenses",life_clothing_store,"displayName"]] call ALFTools_Client_Config_fnc_getConfig;
	private _varnameConfig = ["STRING", ["Licenses", life_clothing_store, "variable"]] call ALFTools_Client_Config_fnc_getConfig;
	private _levelConfigVal = missionNamespace getVariable [format["license_%1",_varnameConfig], false];

	if(!_levelConfigVal) exitWith {
...
if ((_this select 3) isEqualTo "aeroport_parachute") then {
	private _filterCtrl = (findDisplay 3100) displayCtrl 3105;
	_filterCtrl lbSetCurSel 4;
	[_filterCtrl, 4] call ALF_fnc_clothingFilter;
};
...
if(isNil "life_clothesPurchased") exitWith {
	...
	if(backpack player != "") then {
		if(life_oldBackpack isEqualTo "") then {
			...
			if(count life_oldBackpackItems > 0) then {
				{
					[_x,true,true] spawn ALF_fnc_handleItem;
	...
	if(count life_oldUniformItems > 0) then {
		{[_x,true,false,false,true] spawn ALF_fnc_handleItem;} forEach life_oldUniformItems;
	};
	...
	if(vest player != "") then {
		if(life_oldVest isEqualTo "") then {
			...
			if(count life_oldVestItems > 0) then {
				{[_x,true,false,false,true] spawn ALF_fnc_handleItem;} forEach life_oldVestItems;
			};
		};
	};
...
if((life_clothing_purchase select 4) isEqualTo -1) then {
	if(life_oldBackpack != backpack player) then {
		if(life_oldBackpack isEqualTo "") then {
			...
			removeBackpack player;
			player addBackpack life_oldBackpack;
			{[_x,true,true] spawn ALF_fnc_handleItem;} forEach life_oldBackpackItems;
		};
	};
};
