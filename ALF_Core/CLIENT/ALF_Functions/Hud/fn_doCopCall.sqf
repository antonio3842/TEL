...
// Vérifier que le joueur est gendarme en service
// Vérification robuste : s'assurer que les variables existent et sont correctes
if (isNil "life_coplevel" || {!(life_coplevel > 0)}) exitWith {};
if (isNil "life_gendarme_service" || {!life_gendarme_service}) exitWith {};
...
if (isNil "message1_copactive") then { message1_copactive = false; };
if (isNil "message2_copactive") then { message2_copactive = false; };
if (isNil "message3_copactive") then { message3_copactive = false; };
if (isNil "message4_copactive") then { message4_copactive = false; };

// Debug : Vérifier l'état des slots
diag_log format ["[ALERTE GN] Slots disponibles: msg1=%1, msg2=%2, msg3=%3, msg4=%4", message1_copactive, message2_copactive, message3_copactive, message4_copactive];
