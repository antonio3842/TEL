...
if(_name isEqualTo "" OR {_msg isEqualTo ""}) exitWith {hint "Votre message est vide.";};
...
if(_l1 > 30) exitWith {hint "Vous ne pouvez écrire que 30 caractères dans le nom."};
if(_l2 > 200) exitWith {hint "Vous ne pouvez écrire que 200 caractères dans l'annonce."};
...
private _action = ["Voulez-vous publier cette annonce pour 850€ ?","BFM TV","OUI","NON"] call BIS_fnc_guiMessage;
...
["BFM TV","Votre annonce vient d'être diffusée.","success"] spawn ALF_fnc_doMsg;
