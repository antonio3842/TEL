...
if (_title isEqualTo "" OR {_message isEqualTo ""} OR {_notifType isEqualTo ""}) exitWith {};
...
private _color = "";
private _notificationSound = "";
switch (_notifType) do {
	case "success": {
		_color = "#27AE60";
		_notificationSound = "notifSuccess";
	};
	case "warning": {
		_color = "#FFCC00";
		_notificationSound = "notifWarning";
	};
	case "danger" : {
		_color = "#E74C3C";
		_notificationSound = "notifError";
	};
	default {
		_color = _notifType;
		_notificationSound = "notifInfo";
	};
};

if (_title isEqualTo "Central de la Police Municipale.") then {
	playsoundUI ["ALF_Client2\sounds\bip.ogg", 0.25];
} else {
	playSoundUI [_notificationSound];
};
