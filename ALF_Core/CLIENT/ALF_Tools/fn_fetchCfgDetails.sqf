...
if (_section isEqualTo "") then {
	_section = switch(true) do {
		case (_className in ["B_Parachute","MRP_B_Parachute"]): {CONFIG_VEHICLES};
...
// Surcharges masse (kg) — coffres véhicules / mobiliers (vanilla B_Parachute ≈ 160)
private _massOverride = switch (_className) do {
	case "B_Parachute": { 5 };
	case "MRP_B_Parachute": { 5 };
	default { -1 };
};
if (_massOverride >= 0) then { _mass = _massOverride; };
