...
private _display = findDisplay 080520;
...
_returnClass = [[CONFIG_LIFE_VEHICLES, _className]]call ALFTools_Client_Config_fnc_isClass;
...
if ((count _dataArr) isEqualTo 5) then {
	...
	if ((_garage isEqualTo life_garage_id) || (_garage isEqualTo 0)) then {
		...
		_btnsortir buttonSetAction "closeDialog 2; [] spawn ALF_fnc_unimpoundPenit;";
	} else {
		_btnsortir ctrlSetText "Importer";
		_btnsortir buttonSetAction "[] spawn ALF_fnc_importVehiclePenit;";
		_btnvendre ctrlShow false;
    };
};
