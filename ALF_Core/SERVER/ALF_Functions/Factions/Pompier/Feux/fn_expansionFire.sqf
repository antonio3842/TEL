			...
			if !(_alltress isEqualTo []) then {
				...
				for "_i" from 1 to ([2,4] call BIS_fnc_randomInt) do {
					if !(_alltress isEqualTo []) then {
						...
						// Effets visuels clients (mode 0 = gros feu)
						[_feu, 0] remoteExecCall ["ALF_fnc_createFeu", 0, _feu];
						...
						for "_j" from 1 to ([2,4] call BIS_fnc_randomInt) do {
							...
							// Effets visuels clients (mode 1 = feu moyen)
							[_feu2, 1] remoteExecCall ["ALF_fnc_createFeu", 0, _feu2];
						};
					};
				};
			};
