...
_adn = [_adn] call ALF_Server_fnc_numberSafe;
private _face = random[10000000, 50000000, 99999999];
_face = [_face] call ALF_Server_fnc_numberSafe;

private _count1 = ([format["SELECT COUNT(*) FROM players WHERE adn='%1'",_adn],2] call ALF_Server_fnc_asyncCall) select 0;
private _count2 = ([format["SELECT COUNT(*) FROM players WHERE face='%1'",_face],2] call ALF_Server_fnc_asyncCall) select 0;
...
if(_count1 isEqualTo 0 && {_count2 isEqualTo 0}) then {
	...
	[_unit] call ALF_Server_fnc_createAdnFace;
};
