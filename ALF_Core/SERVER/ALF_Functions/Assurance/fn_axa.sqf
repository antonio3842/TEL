...
switch (_m) do {
	case 0 : {
		...
		[_q,1] call ALF_Server_fnc_asyncCall;
		uiSleep 0.5;
		...
		["MACIF","Votre abonnement a bien été résilié.","success"] remoteExec ["ALF_fnc_doMsg",_u];
	};
	case 1 : {
		private _q = format ["UPDATE players SET axa='1' WHERE playerid='%1'",_uid];
		[_q,1] call ALF_Server_fnc_asyncCall;
		["MACIF","Votre abonnement a bien été acheté. 150€ ont été débité sur votre livret A.","success"] remoteExec ["ALF_fnc_doMsg",_u];
	};
	case 2 : {
		private _q = format ["UPDATE players SET axa='2' WHERE playerid='%1'",_uid];
		[_q,1] call ALF_Server_fnc_asyncCall;
		["MACIF","Votre abonnement a bien été acheté. 300€ ont été débité sur votre livret A.","success"] remoteExec ["ALF_fnc_doMsg",_u];
	};
	case 3 : {
		private _q = format ["UPDATE players SET axa='3' WHERE playerid='%1'",_uid];
		[_q,1] call ALF_Server_fnc_asyncCall;
		["MACIF","Votre abonnement a bien été acheté. 700€ ont été débité sur votre livret A.","success"] remoteExec ["ALF_fnc_doMsg",_u];
	};
	case 4 : {
		private _q = format ["UPDATE players SET axabank='0' WHERE playerid='%1'",_uid];
		[_q,1] call ALF_Server_fnc_asyncCall;
		["MACIF","Votre abonnement sera prélevé sur votre Livret A.","success"] remoteExec ["ALF_fnc_doMsg",_u];
	};
	case 5 : {
		private _q = format ["UPDATE players SET axabank='1' WHERE playerid='%1'",_uid];
		[_q,1] call ALF_Server_fnc_asyncCall;
		["MACIF","Votre abonnement sera prélevé sur votre Livret B.","success"] remoteExec ["ALF_fnc_doMsg",_u];
	};
	case 6 : {
		private _q = format ["UPDATE players SET axabank='2' WHERE playerid='%1'",_uid];
		[_q,1] call ALF_Server_fnc_asyncCall;
		["MACIF","Votre abonnement sera prélevé sur votre Livret C.","success"] remoteExec ["ALF_fnc_doMsg",_u];
	};
	case 7 : {
		private _q = format ["UPDATE players SET axa='4' WHERE playerid='%1'",_uid];
		[_q,1] call ALF_Server_fnc_asyncCall;
		["MACIF","Votre abonnement a bien été acheté. 2499€ ont été débité sur votre livret A.","success"] remoteExec ["ALF_fnc_doMsg",_u];
	};
};
