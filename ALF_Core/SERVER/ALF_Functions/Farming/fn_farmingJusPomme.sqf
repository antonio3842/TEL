...
if(_c < 4) exitWith {["INFO","Vous n'avez pas au moins 4 pommes.","warning"] remoteExec ["ALF_fnc_doMsg",_unit];};
...
if(_obj getVariable ["jus",false]) exitWith {
	["INFO","Vous avez récupéré le jus de pomme.","success"] remoteExec ["ALF_fnc_doMsg",_unit];
	["ALF_Jus_Pomme",true] remoteExec ["ALF_fnc_handleItem",_unit];
...
for "_i" from 1 to 4 step 1 do {
	["ALF_Pomme",false] remoteExec ["ALF_fnc_handleItem",_unit];
	uiSleep 0.25;
};
...
if(_unit distance _obj < 8) then {
	["ALF_Jus_Pomme",true] remoteExec ["ALF_fnc_handleItem",_unit];
	_obj setVariable ["jus",false];
	["INFO","Vous avez récupéré la brique de jus de pomme.","success"] remoteExec ["ALF_fnc_doMsg",_unit];
} else {
	_obj setVariable ["jus",true];
};
