...
if(_c < 4) exitWith {["INFO","Vous n'avez pas au moins 4 pommes sur vous.","warning"] remoteExec ["ALF_fnc_doMsg",_unit];};
...
if(_obj getVariable ["XxX",false]) exitWith {
	["INFO","Vous avez récupéré le XxX.","success"] remoteExec ["ALF_fnc_doMsg",_unit];
	["ALF_XxX",true] remoteExec ["ALF_fnc_handleItem",_unit];
...
if(_unit distance _obj < 8) then {
	["ALF_XxX",true] remoteExec ["ALF_fnc_handleItem",_unit];
	_obj setVariable ["XxX",false];
	["INFO","Vous avez récupéré la bouteille de XxX.","success"] remoteExec ["ALF_fnc_doMsg",_unit];
} else {
	_obj setVariable ["XxX",true];
};
