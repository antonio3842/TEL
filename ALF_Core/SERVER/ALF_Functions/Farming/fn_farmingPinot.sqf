...
if(_obj getVariable ["vin",false]) exitWith {
	["INFO","Vous avez récupéré le vin.","success"] remoteExec ["ALF_fnc_doMsg",_unit];
	["ALF_Pinot",true] remoteExec ["ALF_fnc_handleItem",_unit];
...
if(_unit distance _obj < 8) then {
	["ALF_Pinot",true] remoteExec ["ALF_fnc_handleItem",_unit];
	_obj setVariable ["vin",false];
	["INFO","Vous avez récupéré la bouteille de vin.","success"] remoteExec ["ALF_fnc_doMsg",_unit];
} else {
	_obj setVariable ["vin",true];
};
