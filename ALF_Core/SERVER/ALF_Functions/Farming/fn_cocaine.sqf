...
//if(_c <= 3) exitWith {["INFO", "Il n'y a pas assez de Gendarme en service.", "warning"] remoteExec ["ALF_fnc_doMsg",_unit];};//3

private _var = _obj getVariable ["step",0];
switch (_var) do {
	case 0 : {
		if !("ALF_Cocaine_U" in (magazines _unit)) exitWith {["INFO","Vous n'avez pas de pot.","warning"] remoteExec ["ALF_fnc_doMsg",_unit];};
		["ALF_Cocaine_U",false] remoteExec ["ALF_fnc_handleItem",_unit];
	...
	case 2 : {
		...
		if(_dice > 10 && {_dice < 20}) exitWith {
			//[_obj] remoteExec ["ALF_Server_fnc_cocaineInFire",HC_Life];
			[_obj] spawn ALF_Server_fnc_cocaineInFire;
		};
	...
	case 3 : {
		if !(_unit canAdd "ALF_Cocaine") exitWith {["INFO", "Vous n'avez pas assez de place.", "warning"] remoteExec ["ALF_fnc_doMsg",_unit];};
		_obj setVariable ["wip",true];
		["ALF_Cocaine",true] remoteExec ["ALF_fnc_handleItem",_unit];
		[3,3] remoteExec ["ALF_fnc_addXP",_unit];
		[name _unit,getPlayerUID _unit,"COCAINE","vient de récupérer de la cocaine."] spawn ALF_Server_fnc_logIt;
