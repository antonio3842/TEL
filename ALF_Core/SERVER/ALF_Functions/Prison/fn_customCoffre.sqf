...
switch (_mode) do {
	case 0 : {
		...
		[format["UPDATE prison SET safe='1' WHERE id='%1'",_id],1] call ALF_Server_fnc_asyncCall;
	};
	case 1 : {
		_coffre animateSource ["safe_source",1,true];
		[format["UPDATE prison SET safe='0' WHERE id='%1'",_id],1] call ALF_Server_fnc_asyncCall;
	};
	case 2 : {
		_coffre animateSource ["piege_source",0,true];
		[format["UPDATE prison SET piege='1' WHERE id='%1'",_id],1] call ALF_Server_fnc_asyncCall;
	};
	case 3 : {
		_coffre animateSource ["piege_source",1,true];
		[format["UPDATE prison SET piege='0' WHERE id='%1'",_id],1] call ALF_Server_fnc_asyncCall;
	};
};
