...
if(_unit getVariable ["ALF_EnPrison",false]) then {
	...
	[format["UPDATE players SET jailtime='%2' WHERE playerid='%1'",(getPlayerUID _unit),_temps],1] call ALF_Server_fnc_asyncCall;
	[3,[_temps]] remoteExecCall ["ALF_fnc_menuMettrePrison",_unit];
} else {
	//IL N'EST PAS EN PRISON
	[format["UPDATE players SET jail='1', jailtime='%2',jailcoffre='0' WHERE playerid='%1'",(getPlayerUID _unit),_temps],1] call ALF_Server_fnc_asyncCall;
