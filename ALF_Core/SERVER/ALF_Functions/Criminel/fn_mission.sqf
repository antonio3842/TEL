...
private _config = configFile >> "ALF_ConfigClient_Core" >> "ALF_MISSIONS" >> _type;
...
private _spawnDist = _mapSize * 0.7;
private _spawnPos = [(_position#0) - sin(_dir)*_spawnDist, (_position#1) - cos(_dir)*_spawnDist, 275];
private _exitPos  = [(_position#0) + sin(_dir)*_spawnDist, (_position#1) + cos(_dir)*_spawnDist, 275];

private _plane = createVehicle ["JK_C_Cessna_172_F", _spawnPos, [], 0, "FLY"];
...
private _wp1 = _grp addWaypoint [_position, 0];
_wp1 setWaypointType "MOVE";
_wp1 setWaypointSpeed "NORMAL";
private _wp2 = _grp addWaypoint [_exitPos, 0];
_wp2 setWaypointType "MOVE";

// Attendre que l'avion soit proche du point de drop (timeout 5 min)
private _timeout = time + 300;
waitUntil {sleep 1; (_plane distance2D _position) < 300 || !alive _plane || time > _timeout};

// Nettoyage avion en parallèle
[_plane] spawn {
	params ["_p"];
	waitUntil {sleep 5; !alive _p || {(getPos _p) select 0 < 50} || {(getPos _p) select 0 > worldSize - 50} || {(getPos _p) select 1 < 50} || {(getPos _p) select 1 > worldSize - 50}};
	{deleteVehicle _x} forEach crew _p;
	deleteVehicle _p;
};
