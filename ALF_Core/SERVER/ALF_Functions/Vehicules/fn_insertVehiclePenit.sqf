...
if((count _plateresult) isEqualTo 0) then {
	...
	_query = format["INSERT INTO vehicles_penit (classname, type, alive, active, plate, gear, position, damage, id_garage) VALUES ('%1', '%2', '1', '0', '%3', '""[]""', '""[]""', '""[]""', '0')",_className,_type_veh,_plaquefinal];
	[_query,1] call ALF_Server_fnc_asyncCall;
} else {
	[_className,_type_veh,_unit] call ALF_Server_fnc_insertVehiclePenit;
};
