...
private _queryResult = [format["SELECT classname, pid, insure FROM vehicles WHERE plate='%1'",_plaquefinal],2] call ALF_Server_fnc_asyncCall;
...
private _nameplayer = "";
if (_pid isEqualTo "Gendarmerie" OR {_pid isEqualTo "Pompiers"}) then {
	_nameplayer = _pid;
} else {
	private _queryResult2 = [format["SELECT name FROM players WHERE playerid='%1'",_pid],2] call ALF_Server_fnc_asyncCall;
