...
private _vInfo = [format["SELECT id, classname, type, pid, alive, active, plate, gear, fuel, damage, insure, insurecount, peinture FROM vehicles WHERE id='%1' AND pid='%2'",_vid,_pid],2] call ALF_Server_fnc_asyncCall;
...
[format["UPDATE vehicles SET active='1', lockveh='0' WHERE pid='%1' AND id='%2'",_pid,_vid],1] spawn ALF_Server_fnc_asyncCall;

private _gear = [_vInfo select 7] call ALF_Server_fnc_mresToArray;
...
private _damage = [_vInfo select 9] call ALF_Server_fnc_mresToArray;
private _plate = [(_vInfo select 6)] call ALF_Server_fnc_mresToArray;
...
private _ec135 = ["V_ALF_EC135_Gendarmerie","EC135GNlu"];
...
//KeyTimes
private _keys = [format["SELECT pid FROM keysTime WHERE mode='0' AND idobj='%1'",_vInfo select 0],2,true] call ALF_Server_fnc_asyncCall;

//On donne les clés
[_vehicle] remoteExecCall ["ALF_fnc_addVehicle2Chain",(playableUnits select {(([(getPlayerUID _x),_keys] call TON_fnc_index) > -1) OR {(getPlayerUID _x) isEqualTo (_vInfo select 3)}})];
...
private _peinture = [_vInfo select 12] call ALF_Server_fnc_mresToArray;
