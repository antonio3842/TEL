...
private _boundingbox = [_vehicle] call ALF_Server_fnc_getBoundingBox;
...
if (({isNull _x} count _allFires) isEqualTo (count _allFires)) then {
	//[7500] remoteExecCall ["ALF_Server_fnc_updateCNP",HC_life];
	[7500] call ALF_Server_fnc_updateCNP;
	//["BRAVO","VOUS AVEZ ETEINT L'INCENDIE DE VOITURE, FELICITATION !!"] remoteExecCall ["ALF_Server_fnc_doMedCall",HC_Life];
	["BRAVO","VOUS AVEZ ETEINT L'INCENDIE DE VOITURE, FELICITATION !!"] call ALF_Server_fnc_doMedCall;
} else {
	{
		if !(isNull _x) then {
			deleteVehicle _x;
