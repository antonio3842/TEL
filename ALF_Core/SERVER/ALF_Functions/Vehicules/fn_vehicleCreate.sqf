/*
	...
	Author: GYP dev team for Arma Life France
*/
params [
	["_uid","",[""]],
...
private _curConfig = configFile >> "ALF_ConfigClient_Core" >> "plateSettings";
...
if (_texture != "") then 
{
	...
	_peinture = [[_texture,_material]] call ALF_Server_fnc_mresArray;
};

private _letters = ["A","B","C","D","E","F","G","H","J","K","L","M","N","P","Q","R","S","T","V"];
...
if((count _plateresult) isEqualTo 0) then {
	...
	[_uid,_type,_className,_pfinal,_peinture,_secteur] call ALF_Server_fnc_insertVehicle;
	[_uid,_vehicle,1] call ALF_Server_fnc_keyManagement;
...
diag_log format["%1 a  un généré %2 à la position %3",name _unit,_className,getPosATL _vehicle];
