...
private _q = ["SELECT classname, plate, gear, position, fuel, damage, type, peinture FROM vehicles_med WHERE active = '1' AND alive = '1'",2,true] call ALF_Server_fnc_asyncCall;
if(count _q > 0) then {
	{
		private _valeur = _x;
		...
		diag_log format["VEHICULE SP : %1", _valeur];
		...
		{params ["_valeur", "_veh"];
			...
			if (_peinture isEqualType "") then {
				...
				// Tolère des guillemets enveloppants éventuels de la BDD.
				if (
				...
				// Format attendu: ["texture","material"]
				if (
				...
				if (count _parsedPeinture isEqualTo 2) then {
					...
					if (_rawPeinture isNotEqualTo "" && {_rawPeinture isNotEqualTo "[]"}) then {
						diag_log format ["[spawnVehicleLaunch_SP] peinture invalide ignorée plate=%1 raw=%2", _valeur # 1, _rawPeinture];
					};
					_peinture = [];
				};
			};
