...
switch (_mode) do {
	case 0 : {
		...
		_nb = [_nb] call ALF_Server_fnc_numberSafe;
		_count = ([format["SELECT COUNT(*) FROM bank WHERE numacc='%1'",_nb],2] call ALF_Server_fnc_asyncCall) select 0;
...
if (_exit) exitWith {	[_uid,_mode,_unit] call ALF_Server_fnc_achatCompteBancaire;	};
