...
if ((count _result) > 0) then {
	...
	_hsbc = _hsbc + _number;
	[format["UPDATE hsbc SET hsbc='%1' WHERE playerid='%2'",_hsbc,_uidcible],1] call ALF_Server_fnc_asyncCall;
...
_ciblenumberacc = ([format["SELECT numacc FROM hsbc WHERE playerid='%1'",_myuid],2] call ALF_Server_fnc_asyncCall) select 0;
