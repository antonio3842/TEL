...
switch(_mode) do {
	case 0: {
		...
		_value = [_value] call ALF_Server_fnc_numberSafe;
	...
	case 1: {
		...
		for "_i" from 0 to count(_value)-1 do {
			_bool = [(_value select _i) select 1] call ALF_Server_fnc_bool;
			_value set[_i,[(_value select _i) select 0,_bool]];
		};
		_value = [_value] call ALF_Server_fnc_mresArray;
		_query = format["UPDATE players SET licenses='%1' WHERE playerid='%2'",_value,_uid];
	};

	case 2: {
		private _value = param [1,[],[[]]];
		// private _newGear = [_value, false] call ALFTools_Server_MD_fnc_updateGear;
		// _newGear = [_newGear] call ALF_Server_fnc_mresArray;
		_query = format["UPDATE players SET gear='%1' WHERE playerid='%2'",_value,_uid];
	};

	case 3: {
		private _value = param [1,false,[false]];
		_value = [_value] call ALF_Server_fnc_bool;
	...
	case 5: {
		...
		//[_uid,_array,0] remoteExecCall ["ALF_Server_fnc_keyManagement",hc_2];
		[_uid,_array,0] call ALF_Server_fnc_keyManagement;
	};
	...
	case 9: {
		...
		_value1 = [_value1] call ALF_Server_fnc_numberSafe;
		private _value2 = param [3,0,[0]];
		_value2 = [_value2] call ALF_Server_fnc_numberSafe;
		private _value3 = param [4,0,[0]];
		_value3 = [_value3] call ALF_Server_fnc_numberSafe;
