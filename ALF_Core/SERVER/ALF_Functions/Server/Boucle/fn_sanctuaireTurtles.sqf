/*
  ===== CONFIG COMMUNE =====
*/
private _step    = 30;     // bouées tous les X degrés (commun)
private _debug   = false;  // debug compteur (commun)

// [name,              center,                 radius, count, margin, tick, period]
private _zones = [
...
missionNamespace setVariable ["ALF_fnc_randXYInCircle", {
  ...
  private _d = _r * sqrt (random 1);  // uniforme sur le disque
...
missionNamespace setVariable ["ALF_fnc_startBounceAI_server", {
  ...
  [_t,_c,_r,_margin,_tick] spawn {
    ...
    while {alive _t} do {
      ...
      // bord -> demi-tour + petite impulsion
...
/*
  ===== SPAWN 1 TORTUE (serveur) =====
*/

ALF_fnc_spawnOneTurtle_server = {
  // params: [_center,_radius,_margin,_tick]
  ...
  // Tag zone pour respawn ultérieur dans la même zone
...
/*
  ===== RESPAWN (après loot) — remet dans la même zone =====
*/
...
ALF_fnc_respawnTurtle_server = {
  ...
  // Récupère les paramètres de la zone d’origine
...
/*
  ===== COMPTE TORTUES DANS LA ZONE (serveur) =====
  Utilise nearEntities "Animal" car les agents animaux ne sont pas toujours renvoyés par nearestObjects.
*/
...
/*
  ===== BOUÉES + DÉMARRAGE MULTI-ZONES (serveur) =====
*/
if (isServer) then {
  {
    // _zone = [name, center, radius, count, margin, tick, period]
    _x params ["_name","_center","_radius","_count","_margin","_tick","_period"];

    // --- Bouées pour cette zone ---
    ...
    [_name, _center, _radius, _count, _margin, _tick, _period] spawn {
      ...
      while {true} do {
        ...
        if (missionNamespace getVariable ["alf_turtle_debug", false]) then {
          private _msg = format ["[%1] 🐢 %2 / %3   Manquantes: %4", _name, _cur, _count, _missing];
...
/*
  ===== ALERTES POLICIERS (entrée dans zones) =====
*/
if (isServer) then {
  private _zonesRef = +_zones;   // copie locale du tableau défini en tête
  private _cooldown = 60;        // anti-spam (par joueur et par zone)

  [_zonesRef, _cooldown] spawn {
    params ["_zones","_cooldown"];

    private _lastAlertAt = createHashMap;  // "netId|NomZone" -> time
    private _insidePrev  = createHashMap;  // "netId|NomZone" -> bool
...
/*
  ===== DEBUG CLIENT (optionnel) =====

"alf_turtle_debug_msg" addPublicVariableEventHandler {
  params ["_var","_val"];
  systemChat _val; // ou hintSilent _val;
};
