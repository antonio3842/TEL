...
private _type1_detect	= _this # 0;
private _type2_detect	= _this # 1;
private _type3_detect	= _this # 2;
private _rain_drops		= _this # 3;
private _wind_debris	= _this # 4;
private _local_fog		= _this # 5;

if (_type1_detect > 1) then 
{
	[_type1_detect] spawn 
	{
		private _delay1 = _this # 0;
		alias_thunder_1 = true;
		publicVariable "alias_thunder_1";	
	
		for "_i" from 0 to 1 step 0 do 
		{
			if !(alias_thunder_1) then {waitUntil {sleep 0.3; alias_thunder_1};};
			_rand_pl = [] spawn TON_fnc_hunt;

			waitUntil {sleep 0.3; scriptDone _rand_pl};

			_dire	= [2000,-2000] call BIS_fnc_selectRandom;
			_xx = (getpos hunt_alias # 0)+random _dire;
			_dire	= [2000,-2000] call BIS_fnc_selectRandom;	
			_yy = (getpos hunt_alias # 1)+random _dire;
			_zz = 1000;

			// OPTIMISÉ: Suppression publicVariable inutile (la donnée est envoyée via remoteExec)
			private _pos1 = [_xx, _yy, _zz];
			[_pos1] remoteExec ["ALF_fnc_thunderone", 0];
...
if (_type2_detect > 1) then 
{
	[_type2_detect] spawn 
	{
		...
		for "_i" from 0 to 1 step 0 do 
		{
			...
			// OPTIMISÉ: Suppression publicVariable inutile (la donnée est envoyée via remoteExec)
			private _pos2 = [hunt_alias, 700 + random 1300, random 360] call BIS_fnc_relPos;
			[_pos2] remoteExec ["ALF_fnc_thundertwo", 0];
			sleep _delay2;
...
if (_type3_detect > 1) then 
{
	[_type3_detect] spawn 
	{
		...
		for "_i" from 0 to 1 step 0 do
		{
			if !(alias_thunder_3) then {waitUntil {sleep 0.3; alias_thunder_3};};	
