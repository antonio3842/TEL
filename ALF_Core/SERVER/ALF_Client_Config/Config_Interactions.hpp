...
class ALF_CfgInteraction3D {
	...
	class Car_F {
		Interactions[] = {
			...
			{"conducteur","Monter Conducteur","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",2.1,"alive _veh && {(vehicle player) isEqualTo player}","player action ['getInDriver', _target];"},
			...
			//Porte
			{"porte_av_g","Ouvrir Porte","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""porte_avg_source"") isEqualTo 0}","_target animateSource [""porte_avg_source"", 1];"},
			{"porte_av_g","Fermer Porte","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""porte_avg_source"") > 0.5}","_target animateSource [""porte_avg_source"", 0];"},		
			{"porte_av_d","Ouvrir Porte","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""porte_avd_source"") isEqualTo 0}","_target animateSource [""porte_avd_source"", 1];"},
			{"porte_av_d","Fermer Porte","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""porte_avd_source"") > 0.5}","_target animateSource [""porte_avd_source"", 0];"},
			{"porte_arr_g","Ouvrir Porte","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""porte_arrg_source"") isEqualTo 0}","_target animateSource [""porte_arrg_source"", 1];"},
			{"porte_arr_g","Fermer Porte","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""porte_arrg_source"") > 0.5}","_target animateSource [""porte_arrg_source"", 0];"},			
			{"porte_arr_d","Ouvrir Porte","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""porte_arrd_source"") isEqualTo 0}","_target animateSource [""porte_arrd_source"", 1];"},
			{"porte_arr_d","Fermer Porte","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""porte_arrd_source"") > 0.5}","_target animateSource [""porte_arrd_source"", 0];"},			
			{"porte_lat_d","Ouvrir Porte","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""porte_latd_source"") isEqualTo 0}","_target animateSource [""porte_latd_source"", 2];"},
			{"porte_lat_d","Fermer Porte","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""porte_latd_source"") > 0.5}","_target animateSource [""porte_latd_source"", 0];"},			
			{"porte_lat_g","Ouvrir Porte","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""porte_latg_source"") isEqualTo 0}","_target animateSource [""porte_latg_source"", 2];"},
			{"porte_lat_g","Fermer Porte","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""porte_latg_source"") > 0.5}","_target animateSource [""porte_latg_source"", 0];"},
			//Rideaux
			{"rideaux_1","Ouvrir Rideau","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""rideaux_1_source"") isEqualTo 0}","_target animateSource [""rideaux_1_source"", 2];"},
			{"rideaux_1","Fermer Rideau","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""rideaux_1_source"") > 0.5}","_target animateSource [""rideaux_1_source"", 0];"},
			{"rideaux_2","Ouvrir Rideau","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""rideaux_2_source"") isEqualTo 0}","_target animateSource [""rideaux_2_source"", 2];"},
			{"rideaux_2","Fermer Rideau","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""rideaux_2_source"") > 0.5}","_target animateSource [""rideaux_2_source"", 0];"},			
			{"rideaux_3","Ouvrir Rideau","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""rideaux_3_source"") isEqualTo 0}","_target animateSource [""rideaux_3_source"", 2];"},
			{"rideaux_3","Fermer Rideau","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""rideaux_3_source"") > 0.5}","_target animateSource [""rideaux_3_source"", 0];"},			
			{"rideaux_4","Ouvrir Rideau","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""rideaux_4_source"") isEqualTo 0}","_target animateSource [""rideaux_4_source"", 2];"},
			{"rideaux_4","Fermer Rideau","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""rideaux_4_source"") > 0.5}","_target animateSource [""rideaux_4_source"", 0];"},			
			{"rideaux_5","Ouvrir Rideau","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""rideaux_5_source"") isEqualTo 0}","_target animateSource [""rideaux_5_source"", 2];"},
			{"rideaux_5","Fermer Rideau","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""rideaux_5_source"") > 0.5}","_target animateSource [""rideaux_5_source"", 0];"},			
			{"rideaux_6","Ouvrir Rideau","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""rideaux_6_source"") isEqualTo 0}","_target animateSource [""rideaux_6_source"", 2];"},
			{"rideaux_6","Fermer Rideau","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""rideaux_6_source"") > 0.5}","_target animateSource [""rideaux_6_source"", 0];"},		
			{"rideaux_7","Ouvrir Rideau","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""rideaux_7_source"") isEqualTo 0}","_target animateSource [""rideaux_7_source"", 2];"},
			{"rideaux_7","Fermer Rideau","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""rideaux_7_source"") > 0.5}","_target animateSource [""rideaux_7_source"", 0];"},
		    //Brancard
			{"plateau_brancard","Sortir Plateau","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""plateau_brancard_source"") isEqualTo 0}","_target animateSource [""plateau_brancard_source"", 2];"},
			{"plateau_brancard","Rentrer Plateau","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_veh = _this select 0; alive _veh && {(locked _veh) <= 1} && {(_veh animationSourcePhase ""plateau_brancard_source"") > 0.5}","_target animateSource [""plateau_brancard_source"", 0];"},
	...
	// class Land_fonte {
	// 	Interactions[] = {
	// 		{{-0.483398,0.21582,-0.33448},"Fondre l'or","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",1.95,"_object = (_this select 0);(vehicle player) isEqualTo player && {alf_niv_minage > 1} && {({_x isEqualTo ""ALF_Pepite_Or""} count (magazines player)) > 0} && {_object animationPhase ""end"" isEqualTo 0} && {_object animationPhase ""door"" isEqualTo 0}","[player,cursorObject] remoteExec [""ALF_Server_fnc_fonderieOr"",2];"}
	// 	};
	// };

	// class Land_fonte_3 {
	// 	Interactions[] = {
	// 		{{-0.483398,0.21582,-0.33448},"Fondre le cuivre","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",1.95,"_object = (_this select 0);(vehicle player) isEqualTo player && {({_x isEqualTo ""ALF_Pierre_cui_small""} count (magazines player)) > 0} && {_object animationPhase ""end"" isEqualTo 0} && {_object animationPhase ""door"" isEqualTo 0}","[player,cursorObject] remoteExec [""ALF_Server_fnc_fonderieCuivre"",2];"}
	// 	};
	// };

	// class Land_fonte_2 {
	// 	Interactions[] = {
	// 		{{-0.483398,0.21582,-0.33448},"Fondre le fer","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",1.95,"_object = (_this select 0);(vehicle player) isEqualTo player && {alf_niv_minage > 1} && {({_x isEqualTo ""ALF_pierre_fer_small""} count (magazines player)) > 0} && {_object animationPhase ""end"" isEqualTo 0} && {_object animationPhase ""door"" isEqualTo 0}","[player,cursorObject] remoteExec [""ALF_Server_fnc_fonderieFer"",2];"}
	// 	};
	// };
	...
	class canne_sucre_5 {
		Interactions[] = {
			{{0,0,0},"Recuperer la plante","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"(vehicle player) isEqualTo player","[_target] spawn ALF_fnc_takeCanneSucre"}
		};
	};
	...
	class ALF_SlotMachine {
		Interactions[] = {
			// {"jeton","Recupérer Stock","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",2,"_veh = _this select 0; ((vehicle player) isEqualTo player) && b_20","[_target,player,0] remoteExecCall [""ALF_Server_fnc_getStock"",2];"},
			// {"gain","Récupérer gains","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",2,"_veh = _this select 0; ((vehicle player) isEqualTo player) && {_veh animationPhase ""Piece"" isEqualTo 1}","[_target,player,1] remoteExecCall [""ALF_Server_fnc_getStock"",2];"}
		};
	};

	class alf_poker {
		Interactions[] = {
			{"sassoir_1","S'asseoir","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",2.1,"_veh = _this select 0; alive _veh && {(vehicle player) isEqualTo player} && {({!((typeOf _x) in [""ALF_Iphone_Object"",""ALF_3310_Object"",""""])} count (attachedObjects player)) isEqualTo 0} && {({(_x select 2) isEqualTo 0} count (fullCrew [_veh, ""cargo""])) isEqualTo 0}","player moveInCargo [_target, 0];"},
			{"sassoir_2","S'asseoir","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",2.1,"_veh = _this select 0; alive _veh && {(vehicle player) isEqualTo player} && {({!((typeOf _x) in [""ALF_Iphone_Object"",""ALF_3310_Object"",""""])} count (attachedObjects player)) isEqualTo 0} && {({(_x select 2) isEqualTo 1} count (fullCrew [_veh, ""cargo""])) isEqualTo 0}","player moveInCargo [_target, 1];"},
			{"sassoir_3","S'asseoir","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",2.1,"_veh = _this select 0; alive _veh && {(vehicle player) isEqualTo player} && {({!((typeOf _x) in [""ALF_Iphone_Object"",""ALF_3310_Object"",""""])} count (attachedObjects player)) isEqualTo 0} && {({(_x select 2) isEqualTo 2} count (fullCrew [_veh, ""cargo""])) isEqualTo 0}","player moveInCargo [_target, 2];"},
			{"sassoir_4","S'asseoir","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",2.1,"_veh = _this select 0; alive _veh && {(vehicle player) isEqualTo player} && {({!((typeOf _x) in [""ALF_Iphone_Object"",""ALF_3310_Object"",""""])} count (attachedObjects player)) isEqualTo 0} && {({(_x select 2) isEqualTo 3} count (fullCrew [_veh, ""cargo""])) isEqualTo 0}","player moveInCargo [_target, 3];"},
			{"sassoir_5","S'asseoir","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",2.1,"_veh = _this select 0; alive _veh && {(vehicle player) isEqualTo player} && {({!((typeOf _x) in [""ALF_Iphone_Object"",""ALF_3310_Object"",""""])} count (attachedObjects player)) isEqualTo 0} && {({(_x select 2) isEqualTo 4} count (fullCrew [_veh, ""cargo""])) isEqualTo 0}","player moveInCargo [_target, 4];"},
			{"sassoir_6","S'asseoir","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",2.1,"_veh = _this select 0; alive _veh && {(vehicle player) isEqualTo player} && {({!((typeOf _x) in [""ALF_Iphone_Object"",""ALF_3310_Object"",""""])} count (attachedObjects player)) isEqualTo 0} && {({(_x select 2) isEqualTo 5} count (fullCrew [_veh, ""cargo""])) isEqualTo 0}","player moveInCargo [_target, 5];"},
			{"sassoir_7","S'asseoir","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",2.1,"_veh = _this select 0; alive _veh && {(vehicle player) isEqualTo player} && {({!((typeOf _x) in [""ALF_Iphone_Object"",""ALF_3310_Object"",""""])} count (attachedObjects player)) isEqualTo 0} && {({(_x select 2) isEqualTo 6} count (fullCrew [_veh, ""cargo""])) isEqualTo 0}","player moveInCargo [_target, 6];"},
			{"sassoir_8","S'asseoir","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",2.1,"_veh = _this select 0; alive _veh && {(vehicle player) isEqualTo player} && {({!((typeOf _x) in [""ALF_Iphone_Object"",""ALF_3310_Object"",""""])} count (attachedObjects player)) isEqualTo 0} && {({(_x select 2) isEqualTo 7} count (fullCrew [_veh, ""cargo""])) isEqualTo 0}","player moveInCargo [_target, 7];"},
			{{0,0,0},"Jouer au Poker","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",4,"_veh = _this select 0; alive _veh && {player in _veh} && {!(_veh getVariable [""ALF_Poker_InGame"", false])}","[0] spawn ALF_fnc_playPoker;"}
		};
	};
	...
	*/
	...
	class Land_ALF_AtelierMecanique {
		Interactions[] = {
			{"Pont_1_Open","Monter Pont","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_house = _this select 0; alive _house && {(_house animationSourcePhase ""Pont_Elevateur_1"") isEqualTo 0} && {b_Depanneur}","[[""Pont_1_Axis"", ""Pont_Elevateur_1""], true, player] remoteExec [""ALF_Server_fnc_startElevator"",0];"},
			{"Pont_1_Close","Descendre Pont","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_house = _this select 0; alive _house && {(_house animationSourcePhase ""Pont_Elevateur_1"") > 0.5} && {b_Depanneur}","[[""Pont_1_Axis"", ""Pont_Elevateur_1""], false, player] remoteExec [""ALF_Server_fnc_startElevator"",0];"},
			{"Pont_2_Open","Monter Pont","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_house = _this select 0; alive _house && {(_house animationSourcePhase ""Pont_Elevateur_2"") isEqualTo 0} && {b_Depanneur}","[[""Pont_2_Axis"", ""Pont_Elevateur_2""], true, player] remoteExec [""ALF_Server_fnc_startElevator"",0];"},
			{"Pont_2_Close","Descendre Pont","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_house = _this select 0; alive _house && {(_house animationSourcePhase ""Pont_Elevateur_2"") > 0.5} && {b_Depanneur}","[[""Pont_2_Axis"", ""Pont_Elevateur_2""], false, player] remoteExec [""ALF_Server_fnc_startElevator"",0];"},
			{"Pont_3_Open","Monter Pont","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_house = _this select 0; alive _house && {(_house animationSourcePhase ""Pont_Elevateur_3"") isEqualTo 0} && {b_Depanneur}","[[""Pont_3_Axis"", ""Pont_Elevateur_3""], true, player] remoteExec [""ALF_Server_fnc_startElevator"",0];"},
			{"Pont_3_Close","Descendre Pont","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_house = _this select 0; alive _house && {(_house animationSourcePhase ""Pont_Elevateur_3"") > 0.5} && {b_Depanneur}","[[""Pont_3_Axis"", ""Pont_Elevateur_3""], false, player] remoteExec [""ALF_Server_fnc_startElevator"",0];"},
			{"Pont_4_Open","Monter Pont","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_house = _this select 0; alive _house && {(_house animationSourcePhase ""Pont_Elevateur_4"") isEqualTo 0} && {b_Depanneur}","[[""Pont_4_Axis"", ""Pont_Elevateur_4""], true, player] remoteExec [""ALF_Server_fnc_startElevator"",0];"},
			{"Pont_4_Close","Descendre Pont","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_house = _this select 0; alive _house && {(_house animationSourcePhase ""Pont_Elevateur_4"") > 0.5} && {b_Depanneur}","[[""Pont_4_Axis"", ""Pont_Elevateur_4""], false, player] remoteExec [""ALF_Server_fnc_startElevator"",0];"},
			{"Ordinateur_1","Utiliser ordinateur","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_house = _this select 0; alive _house && {(_house animationSourcePhase ""Pont_Elevateur_1"") >= 0.5} && {b_Depanneur}","[[""Pont_1_Axis"", ""Pont_Elevateur_1""], player] remoteExec [""ALF_Server_fnc_startDiagnostic"",0];"},
			{"Ordinateur_2","Utiliser ordinateur","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_house = _this select 0; alive _house && {(_house animationSourcePhase ""Pont_Elevateur_2"") >= 0.5} && {b_Depanneur}","[[""Pont_2_Axis"", ""Pont_Elevateur_2""], player] remoteExec [""ALF_Server_fnc_startDiagnostic"",0];"},
			{"Ordinateur_3","Utiliser ordinateur","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_house = _this select 0; alive _house && {(_house animationSourcePhase ""Pont_Elevateur_3"") >= 0.5} && {b_Depanneur}","[[""Pont_3_Axis"", ""Pont_Elevateur_3""], player] remoteExec [""ALF_Server_fnc_startDiagnostic"",0];"},
			{"Ordinateur_4","Utiliser ordinateur","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",3,"_house = _this select 0; alive _house && {(_house animationSourcePhase ""Pont_Elevateur_4"") >= 0.5} && {b_Depanneur}","[[""Pont_4_Axis"", ""Pont_Elevateur_4""], player] remoteExec [""ALF_Server_fnc_startDiagnostic"",0];"}
		};
	};
	...
	class Land_ALF_Poste {
		Interactions[] = {
			{"Commande","Prendre la commande","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",5,"_table = _this select 0;_service = !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo []);_eta =  player getVariable['ALF_LaPoste_Mission', []] param [3, 0, [0]];if ((alive _table) && !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo [])  && ) exitwith {true};false;","['addColisToPlayer'] spawn ALF_fnc_laposte_fonctions;"},
			{"Commande_2","Prendre la commande","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",5,"_table = _this select 0;_service = !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo []);_eta =  player getVariable['ALF_LaPoste_Mission', []] param [3, 0, [0]];if ((alive _table) && !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo []) && _eta isEqualTo 2 && ALF_LaPosteEnCours && !(missionNamespace getVariable['ALF_colisPris', false])) exitwith {true};false;","['addColisToPlayer'] spawn ALF_fnc_laposte_fonctions;"},
			{"Commande_3","Prendre la commande","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",5,"_table = _this select 0;_service = !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo []);_eta =  player getVariable['ALF_LaPoste_Mission', []] param [3, 0, [0]];if ((alive _table) && !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo []) && _eta isEqualTo 3 && ALF_LaPosteEnCours && !(missionNamespace getVariable['ALF_colisPris', false])) exitwith {true};false;","['addColisToPlayer'] spawn ALF_fnc_laposte_fonctions;"},
			{"Commande_4","Prendre la commande","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",5,"_table = _this select 0;_service = !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo []);_eta =  player getVariable['ALF_LaPoste_Mission', []] param [3, 0, [0]];if ((alive _table) && !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo []) && _eta isEqualTo 4 && ALF_LaPosteEnCours && !(missionNamespace getVariable['ALF_colisPris', false])) exitwith {true};false;","['addColisToPlayer'] spawn ALF_fnc_laposte_fonctions;"},
			{"Commande_5","Prendre la commande","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",5,"_table = _this select 0;_service = !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo []);_eta =  player getVariable['ALF_LaPoste_Mission', []] param [3, 0, [0]];if ((alive _table) && !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo []) && _eta isEqualTo 5 && ALF_LaPosteEnCours && !(missionNamespace getVariable['ALF_colisPris', false])) exitwith {true};false;","['addColisToPlayer'] spawn ALF_fnc_laposte_fonctions;"},
			{"Commande_6","Prendre la commande","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",5,"_table = _this select 0;_service = !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo []);_eta =  player getVariable['ALF_LaPoste_Mission', []] param [3, 0, [0]];if ((alive _table) && !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo []) && _eta isEqualTo 6 && ALF_LaPosteEnCours && !(missionNamespace getVariable['ALF_colisPris', false])) exitwith {true};false;","['addColisToPlayer'] spawn ALF_fnc_laposte_fonctions;"},
			{"Commande_7","Prendre la commande","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",5,"_table = _this select 0;_service = !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo []);_eta =  player getVariable['ALF_LaPoste_Mission', []] param [3, 0, [0]];if ((alive _table) && !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo []) && _eta isEqualTo 7 && ALF_LaPosteEnCours && !(missionNamespace getVariable['ALF_colisPris', false])) exitwith {true};false;","['addColisToPlayer'] spawn ALF_fnc_laposte_fonctions;"},
			{"Commande_8","Prendre la commande","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",5,"_table = _this select 0;_service = !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo []);_eta =  player getVariable['ALF_LaPoste_Mission', []] param [3, 0, [0]];if ((alive _table) && !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo []) && _eta isEqualTo 8 && ALF_LaPosteEnCours && !(missionNamespace getVariable['ALF_colisPris', false])) exitwith {true};false;","['addColisToPlayer'] spawn ALF_fnc_laposte_fonctions;"},
			{"Commande_9","Prendre la commande","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",5,"_table = _this select 0;_service = !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo []);_eta =  player getVariable['ALF_LaPoste_Mission', []] param [3, 0, [0]];if ((alive _table) && !((player getVariable['ALF_LaPoste_Mission', []]) isEqualTo []) && _eta isEqualTo 9 && ALF_LaPosteEnCours && !(missionNamespace getVariable['ALF_colisPris', false])) exitwith {true};false;","['addColisToPlayer'] spawn ALF_fnc_laposte_fonctions;"}			
		};
	};
	*/
	class Land_ALF_Poste {
	...
	class House 
	{
		Interactions[] = 
		{
			{"Door_1_Trigger","Déposer le colis","\a3\ui_f\data\IGUI\Cfg\Actions\take_ca.paa",5,"_house = _this select 0;_var =  _house getVariable['LaPoste_Colis', ''];if (_var isEqualTo (getPlayerUID player)) then {true}else{false};","['returnColis'] spawn ALF_fnc_laposte_fonctions;"},
		};
	};
};
