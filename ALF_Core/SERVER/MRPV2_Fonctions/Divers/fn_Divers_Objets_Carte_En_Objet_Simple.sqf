// MRPV2_Server_fnc_Divers_Objets_Carte_En_Objet_Simple
// Convertit les objets terrain en Simple Objects (incassables + légers)
// OPTIMISÉ: traitement par batch pour éviter les freezes serveur
...
private _liste_Objets_Trier = _liste_Objets_Monde select {
    ...
    private _fileName = toLower ((_modelPath splitString "\") select (count (_modelPath splitString "\") - 1));
    _fileName in _liste_Objets_A_Changer
};

private _totalObjets = count _liste_Objets_Trier;
diag_log format ["[Objets_Carte_En_Objet_Simple] %1 objets à convertir (sur %2 objets terrain scannés)", _totalObjets, count _liste_Objets_Monde];
...
{
    private _obj = _x;
    private _modelPath = (getModelInfo _obj) # 0;
    ...
    if (_modelPath != "") then {
        ...
        private _obj_Simple = createSimpleObject [_modelPath, _pos, true]; // true = local (plus performant)
        ...
        if (!isNull _obj_Simple) then {
            ...
            _errors = _errors + 1;
            if (_errors <= 5) then {
                diag_log format ["[Objets_Carte_En_Objet_Simple] Erreur création: %1", _modelPath];
            };
        };
    };
    
    // Pause tous les X objets pour laisser respirer le serveur
...
diag_log format ["[Objets_Carte_En_Objet_Simple] Terminé: %1 convertis, %2 erreurs, temps: %3s", _converted, _errors, (_timeEnd - _timeStart) toFixed 2];
// };